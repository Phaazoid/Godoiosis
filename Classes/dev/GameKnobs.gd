...
const KNOBS: Array[Dictionary] = [
	...
	{"group": "Unit HUD", "node": "UnitMirror", "prop": "bar_fill_color", "label": "Cube fill",
		"tip": "The health a unit still HAS -- the colour of a cube standing proud of the plate, and of the cubes that fly off when it loses one. Flat: it does not shift hue as the grid empties, since the COUNT already says how hurt the unit is. Fully opaque by design, because this is a gameplay descriptor rather than scenery."},
	{"group": "Unit HUD", "node": "UnitMirror", "prop": "bar_missing_color", "label": "Cube missing",
		"tip": "The health a unit has LOST -- the colour of a cube sunk into the plate. Read together, fill against missing is the whole gauge, so these two want to be as far apart as the palette allows; the recess depth is what keeps it readable for anyone the pair itself does not separate."},
