...
# The terrain fire sheet (#324): FLAME_FRAMES frames left to right, generated by
# tools/lookdev/gen_lookdev_assets.gd. NOT the torch's texture, which is a lamp on a wall and was
# only ever standing in here. The frame count is the sheet's own layout, so it is a const beside
# the path rather than a knob — retuning it would mean re-cutting the art.
const FLAME_TEXTURE_PATH := "res://Art/LookDev/fire_flame.png"
const FLAME_FRAMES := 8

# Flame knobs (aesthetics get a knob, not a guess). See _make_fire for how lift and gap
# interact — the flame is CLAMPED off the ground, the knob cannot sink it back in. Every knob
# BAKED INTO A FLAME rebuilds what is already standing (_rebuild_fires), because the fire poll only
# reconciles when the BURNING SET changes: without that a slider moves and nothing on the board
# does until the next ignition, which is the one failure that makes a tuning knob worthless.
@export var flame_lift := 0.35: set = _set_flame_lift
@export var flame_size := Vector2(0.5, 0.7): set = _set_flame_size

# How many flames a burning cell stands up, and how far off its centre they sit. One quad in the
# middle of a 1x1 square reads as a sprite standing ON a tile; several spread across it read as a
# tile that is ON FIRE (#324). Index 0 is the tall one, the rest ride a ring of this radius.
@export var flame_count := 3: set = _set_flame_count
@export var flame_spread := 0.26: set = _set_flame_spread
...
# cannot separate two vertical coplanar planes — so the separation has to be along the view. Applied
...
# Per-flame animation state, carried on the quad itself (#324): where it stands when nothing pushes
# it, and where in the loop it is. Meta rather than a parallel store, for TUFT_META's reason —
# _fire_markers already tracks lifetime, and a second dictionary would have to follow every free.
const FLAME_REST_META := "flame_rest"
const FLAME_PHASE_META := "flame_phase"
...
# Push every standing flame to where the clock and the camera say it belongs. Public because it is
# the whole animation WIRE and a headless test has no frames to lean on: a clock that ticks while
# nothing reads it looks identical through every count and identity assertion this file already has.
func advance_flames() -> void:
	# Horizontal only: the flames must slide toward the viewer, never climb toward a pitched
	# camera. Null in a headless run and before the rig exists, where no push is the right answer.
	var toward_camera := Vector3.ZERO
	var camera := get_viewport().get_camera_3d() if is_inside_tree() else null
	if camera != null and flame_camera_offset != 0.0:
		var flat := camera.global_position - global_position
		flat.y = 0.0
		if flat.length_squared() > 0.0001:
			toward_camera = flat.normalized() * flame_camera_offset
	for marker: Node3D in _fire_markers.values():
		for child in marker.get_children():
			var flame := child as MeshInstance3D
			if flame != null and flame.has_meta(FLAME_PHASE_META):
				_animate_flame(flame, toward_camera)
				continue
			var light := child as OmniLight3D
			if light != null:
				light.light_energy = flame_light_energy * _flicker_of(marker)


# One quad: the frame window slides along the sheet, and the whole flame is pushed off the plane it
# shares with any unit standing in it (#298). Both are writes to the SAME properties every frame,
# so neither may be authored anywhere else.
func _animate_flame(flame: MeshInstance3D, toward_camera: Vector3) -> void:
	var phase: float = flame.get_meta(FLAME_PHASE_META)
	var rest: Vector3 = flame.get_meta(FLAME_REST_META)
	flame.position = rest + toward_camera
...
# How bright this cell's fire burns this instant. Two sines rather than one, so the light never
...
func _reconcile_state(markers: Dictionary[Vector2i, Node3D], cells: Array[Vector2i],
	...
	for cell in cells:
		...
		if markers.has(cell):
			...
			# The CELL as well as the point: fire seeds its scatter and its animation phase off the
			# cell, so that one tile always burns the same way and two never burn in lockstep. Cover
...
func flame_base_lift() -> float:
	return flame_base_lift_for(flame_size.y)


# The same answer for ONE quad of a cell's fire, which since #324 is not always the authored size:
# the smaller flames of the scatter stand proportionally lower, or a 0.6-scale flame would float
# where the full-size one stands. The GAP does not scale with them — it is a z-fight clearance, not
# a proportion, so every quad clears the ground by the same authored amount.
func flame_base_lift_for(height: float) -> float:
	var lift := flame_lift
	if flame_size.y > 0.0:
		lift = flame_lift * (height / flame_size.y)
	if not flame_writes_depth:
		return lift
	return maxf(lift, height * 0.5 + flame_ground_gap)


func _make_fire(cell: Vector2i, at: Vector3) -> Node3D:
	var root := Node3D.new()
	add_child(root)
	root.position = at
...
# One flame of a cell's fire. The scatter is DERIVED from the cell (a hash, not an rng draw), so a
# tile looks the same every time it is lit and two burning tiles never move together — the same
# determinism the rest of the project holds itself to, applied to a look.
#
# Index 0 stands tall at the middle of the ring; the others are smaller and ride it. Nothing sits
# exactly on the cell centre, which is where the unit sprite's plane passes (#298) — the per-frame
# camera push does the real separating, this only stops them starting out welded.
func _make_flame(cell: Vector2i, index: int) -> MeshInstance3D:
	var count := maxi(1, flame_count)
	var seed_angle := _cell_phase(cell) * TAU
	var angle := seed_angle + TAU * float(index) / float(count)
	var radius := flame_spread * (0.35 if index == 0 else 1.0)
	var scale_of := 1.0 if index == 0 else 0.62 + 0.18 * fposmod(_cell_phase(cell) * 7.0
			+ float(index) * 0.37, 1.0)

	var flame := MeshInstance3D.new()
	var quad := QuadMesh.new()
	quad.size = flame_size * scale_of
	...
	# is fixed by the geometry below, and the sprite half by the per-frame camera push.
	...
	# One frame of the sheet fills the quad; _animate_flames slides the window along it. The
	# emission texture IS the albedo texture, so both follow the one uv1 transform.
	material.uv1_scale = Vector3(1.0 / float(FLAME_FRAMES), 1.0, 1.0)
	...
	flame.set_meta(FLAME_REST_META, Vector3(
			sin(angle) * radius, flame_base_lift_for(quad.size.y), cos(angle) * radius))
	flame.set_meta(FLAME_PHASE_META, fposmod(_cell_phase(cell) + float(index) * 0.41, 1.0))
	flame.position = flame.get_meta(FLAME_REST_META)
	return flame


# A stable number in [0, 1) for a cell — the seed both the scatter and the phase come off. Hashed
# rather than drawn, so it survives a marker being rebuilt and needs no per-cell store.
func _cell_phase(cell: Vector2i) -> float:
	return float(hash(cell) % 1000) / 1000.0


# Re-stand every fire that is already burning. What makes the four geometry knobs LIVE: the fire
# poll only calls refresh_states when the burning SET changes, so on a static board a slider would
# otherwise move nothing until the next ignition. Same shape as _set_tuft_scale's sweep, one level
# up — a flame's size is baked into its mesh, so re-standing is the only way to apply it.
...
func _set_flame_size(value: Vector2) -> void:
	flame_size = value
	_rebuild_fires()


func _set_flame_count(value: int) -> void:
	flame_count = value
	_rebuild_fires()


func _set_flame_spread(value: float) -> void:
	flame_spread = value
	_rebuild_fires()


# Baked into the material AND into the lift clamp, so this one is as dead-until-relit as the four
# above — and it is the switch #298 is argued over, which is the one that most needs looking at
...
func _make_cover(_cell: Vector2i, at: Vector3) -> Node3D:
