...
# A saved capture of the Look tab's scene-mood knobs (#253 part 1): the named moods the dev
# tunes, saves and loads back, replacing the clipboard as the way a look survives a session.
#
# DUMB DATA on purpose. Capturing and applying both need LookTool.KNOBS -- the one authority on
...
# wrong direction.
#
# `values` is keyed by LookTool.preset_key(knob) -- "<node>|<prop>", e.g. "Sun|light_energy" --
...
@export var values: Dictionary = {}   # LookTool.preset_key(knob) -> the captured value
