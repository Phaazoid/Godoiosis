...
const KNOBS: Array[Dictionary] = [
	...
	{"group": "Unit HUD", "node": "UnitMirror", "prop": "number_outline_size", "label": "Number outline", "min": 0.0, "max": 8.0, "step": 0.5,
		"tip": "Thickness of the black outline behind the number, in glyph units rather than cells -- so it holds its proportion when Number size changes. It keeps the digits readable over a bright tile or a lit flame, but it works by DILATING each glyph: push it far and the black copy reads as a second offset character and neighbouring digits bleed together."},
