...
const KNOBS: Array[Dictionary] = [
	...
	{"group": "Unit HUD", "node": "UnitMirror", "prop": "number_outline_size", "label": "Number outline", "min": 0.0, "max": 8.0, "step": 0.5,
		"tip": "Thickness of the black outline behind the number, in glyph units rather than cells -- so it holds its proportion when Number size changes. This is what keeps the digits readable over a bright tile or a lit flame, and readability is worth a heavy value here; it works by dilating each glyph, so only very large settings start bleeding neighbouring digits together."},
