...
enum Kind {
	...
	TALL_GRASS  # #891: grass that carries fire to its CORNERS as well as its sides. Nothing in code
				# keys on it -- both halves of what makes it different are authored on its ignition
				# reaction (TallGrassIgnites.tres), which is why a third flammable ground after it
				# costs a .tres and no edit here.
}
