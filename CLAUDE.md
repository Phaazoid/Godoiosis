...
- **Will / downed / limb-loss (#33) — unblocked scaffold BUILT 2026-06-21 (uncommitted on `main`); the Will resource itself is still a stub.** In code now (all on the transient `Unit`): a `Unit.LifecycleState {ACTIVE, DOWNED, DEAD}` machine — downing happens at the `take_damage` seam (`_select_lethal_rung` → DOWN-at-1-HP vs overkill-dead, gated on the flat `OVERKILL_CEILING` constant, **not** spent Will); squad ejection on down (deferred to after the pass via `game._downed_pending`/`_process_downed_pending`); counter liveness (a unit downed mid-pass skips its resolved counter — `PlanResolver._counter_actor_live` + `ResolvedOutcome.skipped`); the **main-action rule** (attack/rescue mutually exclusive + must follow a move — `BaseAction.is_main_action`/`Unit.has_main_action_queued`); and a **manual `RescueAction`** (adjacent ally spends its main action → `Unit.revive()` to 1 HP, rescued unit spent for the turn). Still design-only (`docs/design/will-and-death.md`): the Will resource + bar, the Law #2 down/maim/lethal preview, maiming, Crisis Mode, the timed-countdown rescue. Forks: 1 resolved (Will persists on `UnitInstance`, #8); **provisional in-code calls** — per-unit Will (no pool), downed-attack = kill, maim deferred. **Design REFRAMED 2026-06-24 (grill — PROVISIONAL, not playtested / not co-dev-reviewed):** Will is now a **limb buffer, not a life buffer** — a sub-overkill fatal hit *always* downs (spending a flat Will cost `N`; **maim** if unaffordable, decided at down-time), and Will **never directly kills** (death = overkill / failed-rescue countdown / Crisis only). **Crisis** = a reactive, high-Will-gated **live interrupt** at a would-be-down → surge + Will→0 + no-safety-net (permadeath) for the rest of that battle. **Generation** = in-fight depletion-only + a **`RallyAction`** (whole turn; restores to a partial cap, less each use) + deferred between-battle recovery (rest + a task-assignment metagame). Full spec: `docs/design/will-and-death.md`.
