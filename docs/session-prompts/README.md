...
1. **Docs are canon.** If code and a design doc disagree, the doc wins. If the doc is *silent*, STOP and ask the user — never invent design. Parked topics (between-battle recovery, temperament, LDR budget numbers, content/naming passes) are **off-limits**: leave a `TODO` pointing at [grill-queue.md](../design/grill-queue.md) and move on.
2. **The collaboration contract is absolute — and it lives in CLAUDE.md, not here.** Read that section; this file no longer restates it. **It changed on 2026-08-05**: Claude writes gameplay code directly, plans first for a new feature or an invasive core-gameplay change, and posts a writeup for small bugfixes. The hand-typed-walkthrough rules these prompts were written under are retired. **The `(user types)` marker in each prompt's header is therefore stale** — read it as a size/scope signal only.
...
- **Between-battle recovery, temperament, affinity expansion, LDR budget, story canon conflicts** — parked grills ([grill-queue.md](../design/grill-queue.md)); no code until grilled. *(The **materia pass** left this list 2026-08-29 — it is grilled, canon is [alchemy-kit.md](../design/alchemy-kit.md) → *Materia*, and it has build tickets: [#694](https://github.com/Phaazoid/Godoiosis/issues/694)/[#695](https://github.com/Phaazoid/Godoiosis/issues/695)/[#696](https://github.com/Phaazoid/Godoiosis/issues/696)/[#697](https://github.com/Phaazoid/Godoiosis/issues/697). The **exotic reagent gate** is the one piece still ungrilled, [#698](https://github.com/Phaazoid/Godoiosis/issues/698).)*
- **Campaign store (A8)** — a named persistence seam, not a build item; it lands with campaign save/load.
- **BREAK banner / visual-clarity work** — ~~rides umbrella [#44](https://github.com/Phaazoid/Godoiosis/issues/44)~~ **dead 2026-08-09: the BREAK doctrine was repealed before the banner was ever built** ([resolution-pipeline.md](../design/resolution-pipeline.md) repeal record); the previewed-fizzle substrate the prompts built serves Law #2 directly.
- **Milestone-A demo blockers that predate the grills** — Balanced archetype, Play-API AI integration (#29 leftovers). Interleave these by taste; 8 covers the AI-ignores-downed piece. *(The #50 fire/ice feel-test passed 2026-07-08 — cleared. **Win/loss detection cleared 2026-07-28** — [#96](https://github.com/Phaazoid/Godoiosis/issues/96) in full (loop, mission select, capture, extraction), canon in [missions.md](../design/missions.md); lose conditions are [#101](https://github.com/Phaazoid/Godoiosis/issues/101).)*
