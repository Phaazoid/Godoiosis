...
```
Project: Iosis (tactical RPG, Godot 4.6, GDScript). Work in C:\Iosis\Godoiosis. Read CLAUDE.md first (the 3 design laws; WeaponData=policy / AttackPattern=geometry; execution order moves -> attacks -> counters), then docs/design/elemental-system.md IN FULL (E1-E8, v1 scope), and docs/design/resolution-pipeline.md IN FULL (the R1-R8 contract — now LOCKED — this build must conform to). Then read the real seams: Classes/Actions/AttackAction.gd (the damage calc in create(), ~line 82), Classes/Managers/SquadManager.gd (calculate_counterattacks_for_squad and get_display_entries_for_squad — the derived-resolver + preview pattern you mirror), unit.gd + UnitInstance.gd (the transient/persistent seam), Classes/ResourceData/WeaponData.gd (elemental_damage_type is already there, stubbed "").
