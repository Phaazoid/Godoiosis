...
DEPENDENCY NOTE (updated 2026-07-28): the mission-end seam NOW EXISTS — #96 built it. Read docs/design/missions.md as canon. Training awards settle where MissionController._end_mission fires; do NOT build a dev-triggered stand-in, and do NOT touch the victory/defeat rules (MissionRules) inside this session. Mission START is the other half of the seam (MissionController.reset) — if a per-mission cap needs zeroing, that is where it goes.
