...
- **Between-battle recovery**; a smarter **AI Crisis policy** than auto-accept (per-archetype stances — prompt 8); **counter-gating** on the two-handed verb lock (deferred to the two-handed weapon content pass — no weapon carries the flag yet).
...
- **Prosthetics pull double duty (audit A2, resolved 2026-07-05).** A prosthetic limb is both a working limb (its built-in stat feeds the slot average, so it wields ordinary weapons like flesh does) and — for weapon-model prosthetics — **equippable as an integrated weapon that consumes no inventory slot** (it's bolted on; expect special-casing). As a weapon it scales off **its own built-in STR, never the unit's** (mixed blends with the unit's other stats are allowed). Weapon face: [weapons.md](weapons.md) → Prosthetic family.
