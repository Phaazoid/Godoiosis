...
- **Verb locks stack on top** (provisional set): any missing arm → **two-handed/heavy patterns locked** + **rescue-carry locked**; off-hand gear interactions TBD with the equipment pass. Legs carry no extra verb — the DEX averaging is the effect.
- **Multi-maim is allowed** (`maimed_part` becomes a set of flags) and **never escalates to death** — a fully-maimed unit still just downs; "Will never kills" stays absolute. Half STR, half DEX (and the MOV band that follows), and no verbs is its own signal to bench and repair.
