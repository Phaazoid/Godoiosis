...
**Deliberately not done.** BLAZE and BURNING still look identical (moot since #890 retired BLAZE) — #174's *one Fire texture covers both* stands, and making the permanent state read fiercer is a design call, not a free win (the expressive version, embers + smoke + haze, is already on Tier 1/3 below). Every flame is a quad and a draw, so `flame_count` is the one knob here with a real cost on a heavily-burning board; the answer if that ever bites is [#311](https://github.com/Phaazoid/Godoiosis/issues/311)'s one-mesh-per-cell, not a smaller number.

### A PARTICLE is one emitter told where to spawn ([#656](https://github.com/Phaazoid/Godoiosis/issues/656), BUILT 2026-09-04)

**The project's first `GPUParticles3D`, and the answer [#551](https://github.com/Phaazoid/Godoiosis/issues/551) exists to stop four tickets each inventing separately.** Its deliverable is the dust a tile throws when it lands in the battle-zoom transition; what generalises past dust is below. The dev's own priorities set the shape: *"making it look cool, without making it difficult to run… I want flexibility, and I don't want performance costs."*

**ONE EMITTER PER EFFECT KIND, NEVER ONE PER CELL OR PER EVENT — and `emit_particle()` is what makes that possible.** `restart()` on a shared emitter kills the burst already in the air, so a per-cell effect otherwise needs a pool of nodes round-robined (`HealthBlockDebris`'s slot shape). `emit_particle(xform, velocity, colour, custom, flags)` spawns at an arbitrary transform on a system that is already resident, so twenty tiles landing across three seconds cost **one node and one draw**. This matters because **per-particle cost is ≈ zero and per-EMITTER cost is real** — a GPU particle's only genuine cost is the compute dispatch and the draw, so the failure mode is forty emitters of eight particles, never one emitter of four thousand. For a CONTINUOUS per-cell effect (embers over every a burning cell, ambient motes) the same rule points at the water mask: `BoardMirror._rebuild_water_mask` already pushes a per-cell texture as a global uniform, and a particle shader can read a board mask the same way rather than growing an emitter per cell.

**Three engine behaviours were MEASURED on 4.7.1 rather than assumed, and each one shaped the design.** All three fail silently, which is why they are written down here rather than left to be rediscovered:

- **`emitting` stays FALSE.** `emit_particle()` fires perfectly well on an emitter that is not emitting; setting `emitting = true` **clears every live particle** on the next emit, so a puff already in the air is wiped. There is no state in which a manually-emitting system wants its process material spawning as well.
- **`amount` is a silent cap, twice over.** It is the live-slot count *and* the emission buffer, and `emit_particle` past it is dropped with no error and no warning — measured: with the buffer full, emitting eight more grains changed the rendered frame by nothing at all. So `amount` is sized **from the knobs that determine the overlap**, never authored, or the effect quietly stops on a busy board. Changing it restarts the system.
- **`ParticleProcessMaterial` overwrites `COLOR` and scale.** Per-grain colour does not survive, so colour and size are **one material value each** and `EMIT_FLAG_COLOR`/`EMIT_FLAG_CUSTOM` are never passed. Position and velocity do survive, which is the half that matters.
- **`visibility_aabb` culls the whole system, and its default is eight cells around the emitter.** This is the one that shipped broken, so it is worth stating as the rule rather than as an anecdote: **a particle emitter draws nothing outside its own box, however correctly it emits** — and a board-scale effect emitted from a node at the origin is outside it by default. Measured: **0 lit pixels** for a puff at the diorama, **1248** for the identical burst inside a box that contains it, with a control puff at the origin proving the mesh and material were right all along. **Derive the box from the volume the effect can reach and re-derive it wherever that volume is written** — for the dust that is `battle3d._board_volume()` merged with itself lifted by `stage_offset()`, since the two arms of the transition are a lift apart. Generously grown: an over-large box costs nothing, and a small one costs the entire effect.

**THE SCATTER IS DERIVED, NEVER `randf()` — and on a GPU system that is a choice `emit_particle` hands you for free.** Because the CPU supplies each grain's position and velocity, the whole scatter is ordinary arithmetic a test can state — `HealthBlockDebris`'s doctrine (*"a deterministic scatter costs nothing, keeps RNG out of presentation entirely, and means a test can assert where a cube went"*) kept intact on a system that is simulated on the card. A seeded `RandomNumberGenerator` is not the thing that rule forbids; the global unseeded roll is.

**A SEED POLICY IS PART OF WHAT AN EFFECT IS, and there are two so far.** The dev asked the question that forced this to be named — *"is there a way to pseudo-randomize the effects? I don't want every particle cloud looking the exact same every time, for some things"* — and the answer is that "every time" means different things per effect:

| policy | seed | who wants it |
|---|---|---|
...
**THE CULL BOX IS WHY "I MAXED THE KNOB AND NOTHING CHANGED" IS A DIAGNOSIS, NOT A COMPLAINT.** Every knob this effect has was tunable, correct and reaching the emitter while the effect drew nothing at all — because the failure was a MULTIPLY BY ZERO downstream of all of them. That signature is worth recognising on sight: when a dial's whole range produces no change, stop tuning and look for the gate, the cull, or the visibility flag between the value and the screen. It is the same shape as #264's born-dead slider read from the other end, and the same shape as the camera-cuts-ahead experiment that made a knob inert in #602 round 3.

**What a test can and cannot see, stated rather than implied by a green run ([#506](https://github.com/Phaazoid/Godoiosis/issues/506)'s law).** A GPU particle is never read back, so no case asserts that a grain was drawn or moved or read as dust — that is the dev's eye, exactly as fire's is. What IS pinnable is every CPU-side decision (the burst function is pure and static), plus **the wire**, which is reachable headless: `_drive_transition` runs off `battle3d._process`, so a flight driven by hand and then given idle frames is advanced by the real driver. The wire case has to leave the crossing frame to the DRIVER — a setup that advances past the landing itself consumes it, and then proves only that `advance_flight` works.

**`advance_flight` names the cells that landed rather than answering yes/no**, and nothing else can: an exit KEEPS its landed cells in `_flight` holding `_flight_to` (#602 round 7), so `flying_cells()` never shrinks and a frame-over-frame diff reports **zero landings on every exit**; the version bumps once per STEP, not per cell. Law #4 — the bool was a lossy projection of the same question.
...
**That lift is a LOCAL delta, so both of its terms belong to the BOARD frame ([#992](https://github.com/Phaazoid/Godoiosis/issues/992), 2026-09-16).** It is `surface_height_at` at the plant's point minus the surface at the cell's CENTRE, and the centre was read off the point the root was placed at — which `BoardMirror.surface_point` had made staging-aware in #521. On a torn-out cell that subtraction evaluates to minus the lift exactly: the root rode 40 up into the diorama and every plant was pushed 40 back down, so the ground left and the grass stayed standing on the board for the whole stage (the dev, on a screenshot: *"tall grass does not join the battle zoom"*). The centre now comes from `BoardSpace.surface_point`, which knows nothing about the tear-out, so the delta is what #342 always described. **The comment on that line never stopped being true — its premise was falsified underneath it**, which is the shape to watch for whenever a seam gains an offset: every ABSOLUTE read that is subtracted from it goes wrong in silence. The Burrow cover bumps are the same decomposition through the same builder and were never wrong, because they leave their own y at zero and let the root carry everything.

**...and the other half of the dev's report was a DIFFERENT bug with the same symptom ([#893](https://github.com/Phaazoid/Godoiosis/issues/893), 2026-09-18), which is worth knowing because #992 read as having closed it.** #992 is settled-stage arithmetic: nothing is moving, a rebuild has already run, and the plant's lift cancelled its own root's. #893 is the FLIGHT: the placement is right at every version bump and nobody re-asks between them, so the whole standing set — props, flames, cover, hole lips — hangs at the height of the last landing while the ground climbs past it. Same screenshot, opposite causes, and fixing either alone leaves the dev reporting the same sentence. `BoardMirror.reseat_cell` is #893's answer and #992 is what makes it one line: **because every part is a local delta under a root carrying the whole of `surface_point`, moving the root moves the plants correctly, so the two fixes compose rather than fight.** Had #992 been fixed by re-adding the offset at the plant instead of asking the delta in one frame, this one would have had to touch every prop shape.

**A lip is the one arm of that re-seat a prop case cannot reach**, and a mutant proved it: deleting it alone reddened nothing across 122 presentation cases. It is keyed on the HOLE's own cell rather than on the ground around it (so it needs the hole itself staged, which a cell attack aimed into a chasm does) and it is the one per-cell node in that file not placed by `surface_point` — built in world vertices spanning shared edges, taking the bare offset. **The general shape: an arm whose INPUT differs from its siblings' is the arm to check a sweep actually covers.**
...
**The door only holds while every writer USES it — and a POOLED GHOST has no state behind the door ([#747](https://github.com/Phaazoid/Godoiosis/issues/747), 2026-09-05).** `UnitMirror.set_ghosts` drove `texture` around `_apply_state_texture()`, and a ghost is built with `.new()` rather than `for_unit_data`, so its sprite triple is empty and the door's own answer is **null**. Two things then had to line up, and both did: **`set_process(false)` in `_init` does not survive entering the tree**, so a node fresh out of the pool ticks `_process` exactly once, and that tick took the *"the animation ran off the end"* branch with nothing ever having played — handing `texture` back to a state that did not exist. The ghost went blank on the frame it was built, in the diorama only, while the flat view drew its 2D twin correctly; and `OverlayMirror._ghost_sync`'s whole-array cache meant no later frame pushed again, so it stayed blank until the move was cancelled and requeued. **Two rules came out of it**: `_process` hands the property back only if it BORROWED it (the gate again, on the release side this time), and the pool writes through `show_still()` — so every *other* `_apply_state_texture` caller is harmless on a ghost too, rather than one of them being the next thing to wipe it. **The observable is not `visible`**: it stayed true throughout, which is why `ghost_count()` and all three existing ghost cases pass with the bug present; the case that pins it reads the sprite's texture and its render base.
...
**All four of those sheets are GONE, and `CardSheet` with them ([#139](https://github.com/Phaazoid/Godoiosis/issues/139), 2026-09-14).** They were the last Fire Emblem rips in the project — [#937](https://github.com/Phaazoid/Godoiosis/issues/937) took the map sprites and this took the battle-scene art, with no replacement, so the zoom has no art at all until [#603](https://github.com/Phaazoid/Godoiosis/issues/603) resumes. Everything above stays as the record of what the format was and why only one sheet yielded to it; **what it is no longer is a description of anything in the tree.** The reader and its generator went because they could only ever read that one dead format — a duration OCR'd off printed digits is not a rule that survives its sheet — and because the dev's own next step supersedes them: he paused this direction wanting *a dev tool to author timings*, which is [#635](https://github.com/Phaazoid/Godoiosis/issues/635) re-scoped. **`SpriteAnimator` and `UnitSprite3D.play_animation` deliberately stayed**: they drive any `SpriteFrames`, so they are unfed rather than dead, and they are the seam #603 comes back to.

**Where a frame HANGS FROM is a second fact about the art, and the sheet is the only thing that can answer it** ([#634](https://github.com/Phaazoid/Godoiosis/issues/634), 2026-08-28). `UnitSprite3D`'s pivot is tuned for the map art — 16 is half of 32, and on a 32x32 map sprite whose ink runs to the texture's bottom edge that lands the origin on the feet. A 66x42 card inherits neither half of that: half of 42 is 21, and the character's column inside the card is **not** the card's centre, because the space is lunge room. On the shipped Sage the idle pose stands at column 18.5 of 66 — most of half a tile out. **The still pivot is `MapSpriteInk.SHEET / 2` since [#937](https://github.com/Phaazoid/Godoiosis/issues/937)** (2026-09-13) rather than a typed 16 -- the sheet is 64 now and the same reasoning holds unchanged, which is exactly why it should never have been a literal.

So the **stand point is measured once, off the idle pose, and stored with the set**: frame 0 of every animation is the stance a gesture returns to, which is verified rather than assumed — all five of the Sage's animations open on a byte-identical frame, and the generator **refuses** rather than picking one if a sheet's idle poses ever disagree. `CardSheet.ground_point` is the rule (split from the writer, same reason as above); the generator measures on the **painted** atlas, because on the source a card is a flat opaque rectangle whose ink bounds are the whole card.

**Only the reference point moves — never the per-frame ink.** That is the viewport rule restated: the sprite's place inside its card is the animation's footwork, and correcting it per frame would delete exactly what the card format exists to carry. Measured on the Sage: the ink's bottom sits at row 41 on all 58 frames while its centre sweeps columns 16 to 46 across one attack.

Two things there were measured against the engine rather than reasoned about, and both would have been wrong by a plausible guess. The origin sits at column `w/2 - offset.x` and row `h/2 + offset.y` — the two axes do **not** share a sign. And **`flip_h` leaves the quad exactly where it is and mirrors only the UVs**, so the horizontal correction has to invert with it: a fixed correction is right facing one way and nearly a full cell out facing the other, which is *worse* than the symmetric error it replaces. (`get_aabb()` reports nothing at all until the sprite is in the tree, which is its own trap — a probe that answers zero for both branches of a fork has answered neither.)

**The pivot is restored inside `_apply_state_texture()`, the one door**, so all three ways a gesture ends — stopped, run off its last frame, interrupted by going down — inherit it, and so will a fourth. A restore beside each caller passes the stopped case and leaves a unit that simply *finished* swinging hanging from a card pivot for ever.

**Still open, and deliberately:** *which* gesture plays *when*, and off what — #603 entire, five forks unpicked. This layer answers only how a frame reaches the screen. Whether a framed blow should anchor to the unit's tile **at all** is that ticket's too: standing where the unit stands is right for the board and may be wrong for a shot a director is composing, in which case #634 becomes the default it overrides rather than the thing it fights.
...
- Particles: dust motes in light shafts, embers above a long-lived fire, drifting leaves/pollen, low ground-fog cards. **Filed 2026-08-26 as [#551](https://github.com/Phaazoid/Godoiosis/issues/551), scoped to the AMBIENT half only** — motes, pollen/leaves, ground-fog cards — because every other clause already has an owner: embers + smoke + haze over A long-lived fire is [#455](https://github.com/Phaazoid/Godoiosis/issues/455)'s (which calls it *"the single most-earned item here"*), rain and snow are [#277](https://github.com/Phaazoid/Godoiosis/issues/277)'s, which punts its own particles forward in as many words, tile debris is [#523](https://github.com/Phaazoid/Godoiosis/issues/523)'s, and the unit channel is [#358](https://github.com/Phaazoid/Godoiosis/issues/358)'s. The measured state that made it P1 rather than someday: **there was not one particle node in the repo** — no `GPUParticles3D`, `CPUParticles3D` or `ParticleProcessMaterial` outside `addons/` — so four open tickets each wanted a particle and not one of them had anywhere to put it. Whichever built first would invent that answer on its way to something else, which is Law #4 failing in its usual way: late, and once there are two. **That is no longer the state, and #551 is not what changed it: [#656](https://github.com/Phaazoid/Godoiosis/issues/656) built the first particle on 2026-09-04** — the dev's own list of wanted effects was the EVENT half (slam dust, fire-damage feedback, sprite states), none of which #551's ambient scope covers, and the entry slam is both the cheapest of them and the one whose every wire was already published by #521. So the seam exists and is written down under *A PARTICLE is one emitter told where to spawn* above; **#551 keeps its ambient deliverable and inherits the conventions instead of declaring them**, and the neighbours read that section rather than re-deriving from this bullet.
- God rays through interior windows (volumetric fog + tight shafts).
- Animated pixel water with real reflections and foam edges (Octopath 2's showpiece). **[#552](https://github.com/Phaazoid/Godoiosis/issues/552) slice 1 SHIPPED 2026-08-26 — a moving, lit, legible surface; foam/shoreline and reflection are separate and #552 stays open.** Three things it settled. **(1) The fork it existed to answer: a cell's own SURFACE is its meshlib item, a MARKER is a per-cell node** ([#324](https://github.com/Phaazoid/Godoiosis/issues/324)'s fire) — so `Scenes/LookDev/water.gdshader` replaces the water items' top material, the GridMap batches every water cell into one draw, and the waves run off **world XZ** so a lake reads as one body of water rather than as N tiles rippling privately. This is the codebase's second `.gdshader` and the convention the next one copies: lit `shader_type spatial` (never `unshaded`, which is what makes the mood presets grade it with no table row), OPAQUE, and it **perturbs `NORMAL` without ever displacing a vertex** — `BoardSpace.lie_on`, `surface_point`, the hover bracket and every markup quad assume a flat top face at a known Y. **(2) [#543](https://github.com/Phaazoid/Godoiosis/issues/543)'s legibility raise is answered from the RULES' own flag** — the generator bakes `deep` from the tile's `walkable`, and the base colours were the two tiles' authored `modulate`, which reached the flat view too (see [terrain.md](terrain.md) → *Water — shallow vs deep*) — **that half is superseded by [#578](https://github.com/Phaazoid/Godoiosis/issues/578) below: 3D water's colour is a knob pair now and the modulate serves the flat view alone**. Measured before the fix and worth keeping: the sheet painted **both** tiles the identical `(77, 155, 230)`. **(3) Water stopped wearing its own surface down its sides** — the one kind that did — and now pairs a top with a generated `water_side.png`, as `grass_top`/`dirt_side` and `stone_top`/`stone_side` always have. Two things it deliberately did not do. A **FROZEN** cell shows moving water under a flat ice icon (`OverlayMirror` mirrors FROZEN as a marker, per [#455](https://github.com/Phaazoid/Godoiosis/issues/455)'s *"genuinely flat"*); the meshlib route carries no per-cell state, so the fix is the same board-sized water mask the foam pass needs — one channel saying *frozen*, and the surface holds still and goes pale. And the shader draws its own faint **cell seam**, because a lake driven off world position erases the grid, and on water the hover bracket is otherwise the only thing saying where a tile ends — `BoardSpace.SIDE_RIM` is built to make that seam invisible and SSAO cannot see a coplanar boundary. Thirteen `GameKnobs` rows tune all of it, as global shader uniforms rather than material parameters, because writing to the meshlib's material would mutate a generated artifact at runtime.
...
- **Slice 2b (2026-08-27) turned the mask from a bitmap into a small FIELD, which fixed two complaints at once.** The dev, feel-checking slice 2: corners of L-shaped water read rough, and the shallow/deep boundary was stark. **Both were the same shape of problem — a value that could only ever be 0 or 1 where the eye wanted a gradient.** (1) **Rough corners were a GRADIENT artefact, not an art one.** Thresholding a *binary* mask through `filter_linear` gives a foam band whose width follows the interpolation gradient — steep across a straight edge, shallow near a corner — so an L-shaped patch grew a fat mushy corner while its straight runs stayed crisp. R is now a **signed distance to the shore**, built by a two-sweep chamfer transform in `_rebuild_water_mask` (CPU, once per terrain edit), encoded 0.5 exactly on the boundary and saturating `SHORE_RANGE` cells out. A real distance field has a **constant gradient, so the band has a constant width** — that is the whole fix, and on a straight shore the 0.5 crossing lands exactly where the bitmap put it, so straight runs are a visual no-op. The shader also samples the mask at the **art pixel's centre** rather than the fragment's, so the shoreline stair-steps like the bands and the bed instead of being the one smooth anti-aliased curve on a 32-pixel-per-cell surface. **The honest limit: two water cells touching only diagonally still meet at a point** — that is geometry, not encoding. (2) **The stark depth seam was structural: `deep` was a per-MATERIAL uniform**, baked 0 or 1 by the generator from the tile's `walkable` flag, so all eleven `mix(shallow, deep, deep)` sites snapped at the cell edge and the bed popped from present to absent in one pixel. **There was no code path that could blend.** G is now deepness in the same mask, the per-material uniform is **deleted**, and the two water materials collapse to **one** — because a bake beside a mask channel is two answers to *how deep is this water*, the duplicate-seam shape that fails late. At a cell centre G is exactly 0 or 1, so away from a type seam this reads identically to the baked version; at a seam all eight knob pairs glide across and the bed fades out *into* the deep. **The subtlety that makes it work: land texels take the deepness of the water beside them** (one 8-neighbour dilate, deeper wins a tie) — without it, bilinear from a deep cell's centre reaches into its land neighbours and drags every cliff rim toward shallow, fading the bed in against the wall. **Residual, stated rather than discovered:** the two tiles' authored `modulate` still steps at the cell edge, because that is tile art and no shader can unbake it per fragment. **And `walkable_of`'s justification got SHARPER rather than weaker**: the generator was its second caller and no longer reads it, but `BoardMirror`'s mask does — and the render needs the tile TYPE's flag precisely because a FROZEN water cell is *walkable* while still being deep water to look at, which `BoardContext.is_walkable` cannot say. The decode scale (`water_board_shore_range`) is **pushed rather than spelled in both languages**, and joining `BOARD_GLOBALS` is the closed exemption list from slice 2 being widened *deliberately* — the mechanism working as designed rather than around.
- **[#646](https://github.com/Phaazoid/Godoiosis/issues/646) (2026-08-28) — the rule slice 2b's gliding seam was missing, and it generalises to every shader that blends two parameter sets.** Making all eight knob pairs `mix()` on the mask's depth value is right for the amplitudes and **wrong for the two that set PHASE**. A sinusoid's frequency and phase rate do not linearly interpolate: with `scale` a function of position the surface becomes a spatial **chirp**, and with the phase *rate* a function of position it becomes one that tightens forever. Phase is `(p.x + p.y) * scale`, so a glide of 1.4 across the one-cell transition crammed **~6.7 extra periods** into that cell at board `(20, 10)` — worse the further the lake sits from the world origin — while the speed difference added `TIME × 0.94` on top, **unbounded**. The dev reported it as *"horrible aliasing trying to merge them"*, and it was neither aliasing nor a mis-tune: the bands, the shoreline wobble and the specular all ringed at once because all three read the same wave value, and past a couple of minutes the rings went finer than an art pixel and became moiré. **A PARAMETER THAT SETS PHASE CANNOT BE INTERPOLATED ACROSS SPACE; ONE THAT SETS AMPLITUDE CAN** — so `wave_at` evaluates BOTH fields and interpolates the *result*, which is an amplitude, and the caustics run at shallow's own pace since the bed is a shallow-only layer already gated on `(1.0 - d)`. Cost is one extra `wave()` per sample. **Declared residual:** inside the transition cell the two fields crossfade, so band contrast dips slightly there; not pre-compensated, because that is a feel call. **Measured rather than argued** — the shader and `_rebuild_water_mask` were ported to an offline renderer, and band edges crossing the transition went 11/13/14 at TIME 0/30/120s to 3/2/1. `PHASE_GLOBALS` in `tests/law/test_water_looks_as_deep_as_it_is.gd` is the closed category, the `BOARD_GLOBALS` shape, and **it says out loud what it cannot catch**: it pins the `mix()` spelling, so a hand-rolled `a * (1.0 - d) + b * d` chirps identically and passes — confirmed by a mutant rather than asserted, per the rule that a law overselling its reach is worse than no law.
- **[#578](https://github.com/Phaazoid/Godoiosis/issues/578) (2026-08-28) — the colour half, and it cost the *"one number, both views"* line above.** Slice 2b left a stated residual: the two tiles' authored `modulate` still stepped at the cell edge, because a shader cannot unbake a per-tile tint per fragment — by `fragment()` the two tints are two texels in one atlas. The dev's eye confirmed it within a day and again on the #646 report. **The fork was put to him rendered rather than described** (an offline port of the shader, four options, real tints decoded out of `ground_atlas_0.png`), and he chose the honest-cost answer over the tidy one: **the shader owns water's colour, two knobs, and the flat view keeps the modulate — a DECLARED [#292](https://github.com/Phaazoid/Godoiosis/issues/292) divergence.** So the generator composes water untinted, water's top face stopped sampling the atlas, and the `atlas` sampler is DELETED from the shader — a tint left baked there would have had no reader, which is the *declared but unread* smell this file's own law exists for. `test_the_composed_atlas_carries_each_water_tile_s_authored_tint` INVERTED into `..._leaves_water_untinted`, and its teeth are the non-vacuity guard: some water tile must still AUTHOR a modulate, or "untinted" is evidence of nothing to skip rather than of a skip. **The second half is that G stopped being a FLAG.** It was 0-or-1 with a one-cell bilinear glide, so the depth ramp could only ever be a cell wide however it was tuned; it is a signed chamfer distance from the shallow/deep seam now — R's own trick, one boundary along — encoded over a fixed range and **decoded by a knob, so the width is tunable without rebuilding the mask**, exactly as foam width already was. Widening it moves **every** per-type pair, which is what makes the two waters read as one body that deepens rather than two kinds meeting; 0.5 reproduces the old step. A third channel (B, distance from land) drives water losing light as it leaves the shore — the one depth cue that is DARK rather than bright, which is the direction his whole #552 sweep moved. **B could not reuse R**: R is encoded over two cells precisely so the foam band keeps sub-art-pixel precision, and stretching it to carry a depth cue would step the surf visibly. Two of the six new knobs name **neither** water — they describe the transition between them — so `SHARED_GLOBALS` joins `BOARD_GLOBALS` as a second declared exemption from the naming law, closed list, membership asserted both directions.
...
- **Impact frames — an OPTION in the battle zoom's eventual control set, not an addition to the zoom (dev ruling, 2026-09-12 scratchpad sweep).** The entry was three words, *"Impact frames in battle zoom"*; asked which half he meant, he declined the framing: *"Rather than treat this as an immediate thing to add to the battle zoom, I think this should be treated as an option to add to the eventual more thorough set of controls I'll want over the battle zoom."* So the wanted thing is the anime/fighting-game technique — a high-contrast flash or abstract silhouette drawn OVER the moment of contact for a frame or two — and the wanted shape is a thing the player can turn off. **The half that is easy to mistake for it already ships:** [#520](https://github.com/Phaazoid/Godoiosis/issues/520) diff 2c built the impact FREEZE (`Pacing.hitstop`, `Engine.time_scale = 0.0`, one caller at `battle3d._on_impact`). What is worth recording is that **the control set he is describing already has an inhabitant waiting for it** — `CINEMATIC_HITSTOP` / `BOARD_HITSTOP` are dev knobs picked by [#519](https://github.com/Phaazoid/Godoiosis/issues/519)'s profile, so the player's only say over the whole impact moment today is [#647](https://github.com/Phaazoid/Godoiosis/issues/647)'s three-way cinematic mode, which is on/off/combat-only for the entire zoom. A per-effect control set is therefore the #394 shape (*the player SCALES what the dev authored*, above) aimed at playback rather than at the camera, and the first question it has to answer is whether each row is a toggle or a scale. The effect itself would draw through the [#887](https://github.com/Phaazoid/Godoiosis/issues/887) blow-landed wire and belongs with [#603](https://github.com/Phaazoid/Godoiosis/issues/603)'s sprite-side blow, not beside it.
