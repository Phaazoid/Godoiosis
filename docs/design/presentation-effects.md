...
- Animated pixel water with real reflections and foam edges (Octopath 2's showpiece). **Filed 2026-08-26 as [#552](https://github.com/Phaazoid/Godoiosis/issues/552)**, and it is further from started than the line suggests. `Terrain.Kind.WATER` is MeshLibrary item 5, built as `_block_mesh(_mat(water_top), _mat(water_top))` — a solid opaque cube wearing the *same* static texture on its top face and its sides, the only kind that does not pair a top with a distinct side material. `_water_top()` draws a flat blue with row banding and fourteen two-pixel flecks, once; `_mat()` sets `roughness = 1.0`, so the one property that would let water catch a highlight is dialled to its opposite, uniformly with dirt and stone. **[#543](https://github.com/Phaazoid/Godoiosis/issues/543) then raised this from aesthetics to legibility**: shallow water is ordinary ground and deep water drowns, and `SHALLOW_WATER` is an atlas coord rather than a `Kind`, so two mechanically opposite tiles are told apart by top-face art alone, at tactical camera distance. Worth stating the size honestly — the project contains exactly one `.gdshader`, so this is the first surface-shader work here and whatever it does becomes the convention every later shader copies.
