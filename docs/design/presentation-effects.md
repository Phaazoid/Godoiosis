...
### The first real gesture: a swing the sprites carry ([#603](https://github.com/Phaazoid/Godoiosis/issues/603) slice 1, [#635](https://github.com/Phaazoid/Godoiosis/issues/635)'s Brigand half, 2026-09-05)

#629 built playback and stopped before beats. This is the first gesture the game actually plays on its own: **a unit whose art family has an animation set swings it when it attacks.** Everything else about a blow is unchanged, which is what makes it shippable while three of the four sheets are still unreadable.

**A LOOSE sheet is RECONSTRUCTED, not read, and that is a different promise.** `LooseSheet` is `CardSheet`'s sibling rather than a mode of it, because the two answer differently. A card sheet is a fixed viewport and therefore *carries* the animation's footwork; a loose one was ripped to tight bounding boxes and laid out by hand, so the only thing left to align frames by is the ink itself. Frames are 8-connected regions of not-page ink — 8-connected because an axe blade meeting a hand diagonally is one frame and a 4-connected fill files it as two, which is the opposite of what `CardSheet` needs and why the fill is a second function rather than a parameter on the first. They are then **re-carded**: ink bottom flush with the card's ground line, ink centred across it.

**What re-carding gets right, and what it cannot.** Vertical is right by construction — the lowest ink *is* the ground, so the Brigand's leap keeps its height off its own drawn shadow with nothing told about jumping. Horizontal drifts with the bounding box: an axe swung out to one side widens the box that way and walks the body the other. **Measured on the shipped sheet, the worst frame-to-frame body shift is about 10 texels, a third of a cell**, at the two frames where the swing arc appears. Two alternative anchors were measured against it — the ink centre of the bottom rows, and of the lower half — and **both were worse**, with 6–9 texel steps of their own. There is no rule that recovers registration a rip discarded; hand-authored per-frame offsets are the only real cure and they are #635's own fork 2, unpicked.

**Timing is authored in the manifest, because the sheet prints none.** The Sage's durations are read off its cards; the Brigand's twelve are typed by hand — 74 GBA frames, ~1.23 s, the axe landing on frame 8. The asymmetry is the sheet's, not a design choice.

**The swing crosses to 3D as a VERSION-GATED POLL, not a signal.** `UnitVisuals` counts its lunges and `UnitMirror` compares that count against its own last-seen value — #308's idiom, and the mirror keeps death as the one signal it listens to. A poll of the offset cannot answer this: the invalid-order shake moves the same channel, and a tween that has run its course is indistinguishable from one that never fired. **The ledger is seeded from the unit's own count when a sprite is built**, never from zero, or a unit that reaches the mirror after its first blow — a mid-pass spawn, a #739 redeploy — replays a swing already struck on the frame it appears.

**Which set belongs to a piece of art now has one home**, `UnitSprite3D.zoom_set_for`, keyed on the map sprite and **caching its misses**: most families have none and the question is asked on every blow of every battle, so a remembered null is the difference between asking the disk once and asking it forever. Where a unit's animations are *authored* is still #603's fork 3 and still a convention rather than a field.

**Declared, and not a bug to be found later: the beat still fires damage before the axe lands.** `AttackAction` awaits the 0.18 s 2D lunge and then applies damage, so the camera jolt and the health cubes land roughly 0.45 s before frame 8. Making a beat wait for a gesture's real length is #603's forks 1 and 2. One consequence is worth naming because it *repeals a declared cut*: #520 2c recorded that "freeze mid-swing" could not exist because the sprite was back at rest by the time the death signal fired. With a 1.23 s gesture running past a 0.18 s tween, **there is now a swing to freeze mid-** for any family with a set.

**Still open, and deliberately:** *which* gesture plays *when*, and off what — #603's five forks, four still unpicked after slice 1 above. It answers only *the attacker swings when it attacks*; whether a gesture reads the beat's DRAMA, whether the VICTIM recoils, WHERE animations are authored and whether any of it forks on the zoom PROFILE are all still open. Whether a framed blow should anchor to the unit's tile **at all** is that ticket's too: standing where the unit stands is right for the board and may be wrong for a shot a director is composing, in which case #634 becomes the default it overrides rather than the thing it fights.
