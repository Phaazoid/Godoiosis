...
- **I6.** Members must stand within the **leader's cohesion radius** (`Stats.Stat.COH`, Manhattan — per-unit since [#142](https://github.com/Phaazoid/Godoiosis/issues/142), still decoupled from LDR; banner). Read only via `Squad.get_max_squad_range()` — never a member's own COH. After leader reassignment, out-of-range members are detached into solo squads (`check_reassign_leader`), and since the successor's COH may be shorter, a swap can detach members the old leader held.
