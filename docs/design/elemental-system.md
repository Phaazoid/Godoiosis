...
- **Element** (`enum Element`) — a tag on an *outgoing hit*; what an attack *is* (FIRE, WATER, SHOCK, ICE…). An attack carries a **set** of elements — usually one, but **not limited to one** (multi-element is supported by the model; first content is single-element). Lives on the attack/weapon. **CORROSION joined the roster 2026-09-06 ([#97](https://github.com/Phaazoid/Godoiosis/issues/97)) and is INERT — an exotic like ICE and SHOCK (never a sigil, never aura-bearing), with no reaction, no paired `State` and no terrain source.** It exists because the Chemical Spitter's identity is corrosion and its Spray had nothing to be tagged with; the toxin design it is eventually for is still an idea-bank entry in [elemental-interactions.md](elemental-interactions.md). Note it shares a NAME with `AttackData.Kind.CORROSION` and is a different axis entirely — the same overlap FIRE and SHOCK already have, so a grep for the word hits both.
...
- **Tile / terrain states.** *(Slice 1 BUILT 2026-06-28 — `TerrainStateManager` store + the `PlanResolver` cell-effect channel + a first `FIRE×tree→BURNING` reaction, in a **separate** `Terrain` vocabulary; `tests/terrain/`. See terrain.md build status.)* The store is a parallel `Dictionary[Vector2i, …]` in `TerrainStateManager`, persisted in `ScenarioData` (round-trip pending) and drawn by `OverlayManager` (pending). De-randomized wiki taxonomy worth keeping:
  ...
  - ~~Pathing applies state: stepping on a river tile sets WET~~ — **BUILT 2026-09-10 ([#884](https://github.com/Phaazoid/Godoiosis/issues/884)).** `RulesService.wets_in` is the rule (WATER kind, not FROZEN, no Waterwalk — shallow and deep alike, since depth is walkability and wetness does not read it), asked in `PlanResolver.resolve_move`'s per-cell walk and at a shove's LANDING. It lands in the RESOLVER rather than at execution, which is the whole point: the soaking is threaded into the hypo, so a SHOCK hit later in the *same* pass sees it (E4) and the queue shows the electrocute before Execute. The *"optional custom pathing"* half — a mover preferring to keep its feet dry — is NOT built and belongs with [#419](https://github.com/Phaazoid/Godoiosis/issues/419)'s still-open hazard-pathing fork, which is the same question about a different hazard.
...
- **Some elements bypass DEF — RESOLVED by [#424](https://github.com/Phaazoid/Godoiosis/issues/424) (2026-09-06), and not as a bypass.** The 2026-07-29 framing (*"Fire, shock could ignore defense, whereas pelting someone with blocks of earth or ice shards would still be blocked by defense"*) became a **damage KIND on every attack** — `AttackData.Kind`: blunt / slash / pierce / fire / shock / cold / corrosion, AUTHORED and never derived (an ice spear is PIERCE, sheer cold is COLD); the element set stays the separate EFFECT axis, so *a fireball deals fire-kind damage and separately applies the fire effect* — plus **armour listing the kinds its DEF covers** (`ArmorData.covered_kinds`, through `RulesService.def_against` in the one mitigation stage). Plate covers the physical three, so a fireball goes through it and an earth block does not: content, not a mechanism. Reading (d), a physical/elemental split of the number, is dead — one kind per attack, one number. Left open by ruling: a per-kind DEF *table* (*"too early to make a call"*); the list is its subset. Insulation itself became EFFECT-strip only in the same pass — the never-arrives outcome is repealed ([stats.md](stats.md)). **That rule governs a DELIVERED hit, and [#892](https://github.com/Phaazoid/Godoiosis/issues/892) (2026-09-11) says so out loud:** the end-of-turn tile burn carries no kind and DEF never sees it, so there is no "let DEF answer the damage" half available there and fire insulation stops the burn outright ([terrain.md](terrain.md)). FIRE is the second element with an insulation ability authored.
- **Alchemic wards — COLLAPSED at the #424 grill (2026-09-06) into [#799](https://github.com/Phaazoid/Godoiosis/issues/799).** Once armour DEF applies per kind and FIRE / SHOCK / COLD / CORROSION are kinds, an inscribed ward is a DEF row covering a non-physical kind (authorable today) and a channeled one is a temporary DEF term in `def_breakdown` — the only unbuilt piece. No new mechanism, and the name collision with a Guard's ward evaporates because nothing needs the word.
