...
> **Build status — #50 DONE + CLOSED (2026-06-28 → 06-30, three sessions).** The dynamic per-cell state store exists: `TerrainStateManager` (`Dictionary[Vector2i, Array[Terrain.TileState]]`), with `Terrain.TileState {BURNING, FROZEN, COVER, BLAZE}` (COVER added by Burrow, #84 — see *Cover* below; BLAZE by #174 — the banner below) and a `Terrain.Kind` enum (GRASS/MUD/ROCK/TREE/WATER/DIRT/VOID — DIRT added 2026-08-12 as the non-flammable ground, see the thermal-batch banner below; VOID added by #259, 2026-08-20 — the unwalkable nothing a shove flies over and a shove *ending* on removes, see `verticality.md`; since [#875](https://github.com/Phaazoid/Godoiosis/issues/875) (2026-09-09) it is not the only spelling of a hole — **no ground inside `grid.get_used_rect()` is one too**, and `BoardContext.is_void_at` is the one reader of that question, deliberately NOT folded into `terrain_kind_at`, which every content system asks for what the AUTHOR wrote) read straight off the tileset's `terrain_type` int custom-data layer (`Resources/TestTiles.tres` — an int layer as of [#71](https://github.com/Phaazoid/Godoiosis/issues/71); the string->enum mapping boundary it used to need is gone). Fed by the resolver's **cell-effect channel** (`ResolvedPlan.cell_effects` / `ResolvedCellEffect`, populated by `PlanResolver` when given a board), gated by the per-attack `EquippableData.TargetMode` toggle (unit / map / both, default unit). `Terrain` is a **separate vocabulary** from `Elemental` (dev call). **All three planned slices shipped:** slice 1 (headless plumbing), slice 2 (live execution + queue preview), slice 3 (`ICE × water → FROZEN`, `FIRE × FROZEN → water`, both authored `.tres` reactions). Also shipped beyond the original scope: AoE-footprint deposit (every affected cell, not just the aim cell), counters depositing too, persistence (`ScenarioData.terrain_states` — see below), burnout after 3 turn cycles (`STATE_DURATIONS`/`tick_states` — BURNING only; FROZEN's own clock was removed 2026-08-12, see the banner below. **`STATE_DURATIONS` is GONE as of #890** — the clock is the GROUND's now, and three turns is what grass authors rather than what fire is), and burning-tile damage on end-of-phase (routed through `take_damage`, so downs/Crisis apply correctly). Proven in `tests/terrain/{test_cell_effects, test_terrain_persistence, test_burnout, test_ice}.gd`. **Deferred by design, not gaps:** burning spread (**BUILT by #890, 2026-09-10** -- see its banner below) + a varied elemental-effect roster beyond fire/ice (still deferred). *(The fire-only plan-time ghost preview noted here as deferred was generalized 2026-07-24 by #84's Burrow — `show_terrain_preview` now takes `{cell, state}` entries and draws each deposit's own icon, matching the live overlay.)*

> **#174 (2026-08-10): authoring + the second fire.** `Terrain.TileState.BLAZE` — authored set-dressing fire, BURNING's permanent sibling (no `STATE_DURATIONS` entry, COVER's exact mechanism), same end-of-turn damage. *Is this tile on fire?* now has **one spelling**: `Terrain.FIRE_STATES` / `Terrain.is_burning`, with `TerrainStateManager.burning_cells()` the enumeration form — a cell legally holding both fire states (painted BLAZE, then a fireball) burns its occupant ONCE. *(#419, 2026-09-01: `burning_cells()` is now read by the RENDERERS, which want fire specifically. The end-of-turn burn walks UNITS and asks `RulesService.occupant_damage_for` (one layer up since #892, adding whether that unit is fireproof) at each one's cell, so it stays symmetric with the queue's forecast — see* Hazardous tiles and pathing *below.)* Authoring surfaces: the dev Tile Brush gained a **Tile States paint mode** (every non-NONE state paintable — authored map Cover is now real; painted BURNING carries its real 3-turn clock, permanent fire is what BLAZE is for — **both halves REPEALED by #890**: there is one fire, and how long it burns is the ground's answer) and the Unit Editor gained **live element-state toggles** (soak a unit without authoring a WATER attack). Two load-path gaps this exposed are closed: `apply_scenario` redraws terrain state on load (authored fire is visible at turn one), and `clear_board` clears the state store, which nothing had ever done.
...
> **2026-08-12: the fire/water terrain loop closed (elemental thermal batch).** Water puts fire out — `DouseBurning.tres` (**and `DouseBlaze.tres`, DELETED by #890 with the state it was for** — the douse gates on the STATE, so one reaction covers a fire on any ground: the ice symmetry, a permanent state removed by its opposing element, never a clock). Fire spreads to ground — `GrassIgnites.tres` (FIRE × GRASS → BURNING, joining the tree ignite). And `Terrain.Kind.DIRT` is the **non-flammable default ground**: no reaction keys on it, by design, so level authors can paint firebreaks (`tests/terrain/test_douse.gd` pins the omission). *(DIRT had no tileset tile until #554 authored the first three — see the banner below.)*
...
>
> **[#890](https://github.com/Phaazoid/Godoiosis/issues/890) part 1 (2026-09-10): a fire's clock belongs to the GROUND, and BLAZE is RETIRED.** The dev's ruling, filed against fire spreading: *"that clock is variable depending on what is burning. Some things can burn forever, some last 3 turns, some one. This also lets us generalize the blaze implementation — it can just be a normal fire on a material that burns forever."* So the duration left `TerrainStateManager.STATE_DURATIONS` (a table keyed by TileState, **deleted** rather than demoted to a default — both live construction sites wire the replacement, so a fallback would be a constant only tests could reach) and became **`TerrainReaction.add_state_turns`**, `ElementalReaction`'s exact grammar applied to the map: **ABSENT = no clock = permanent**, which is already how COVER and FROZEN spell it, so there is no "forever" sentinel to get wrong. `TerrainReactionCatalog.fuel_for_kind` is THE spelling of *what does fire consume on this ground* — asked of the authored reactions, so flammability stays `required_kind` on a `.tres` and never becomes a list of kinds in code — and `TerrainStateManager.fuel_source` is the injected callable, `ground_source`'s twin at the same two sites.
>
> **The load-bearing clause: fire on ground with NO ignition reaction burns until doused.** It is consuming nothing, so nothing runs out — which is exactly what the twelve authored braziers on Prolog's flagstones have always meant, and is why retiring BLAZE needed no new material and no re-authoring. **`Terrain.TileState.BLAZE` stays in the enum as a TOMBSTONE** (it serializes as a plain int, and a hole is worse than a dead member); `Terrain.RETIRED_STATES` is what keeps it out of the brush palette and the Glossary law, and `load_state_dict` drops it at the one door an old save can bring one through, so no reader downstream has to know the enum has holes in it. `DouseBlaze.tres` is **deleted** — `DouseBurning` gates on the STATE, so one douse covers both kinds of fire — and Prolog's twelve cells swept `[4]` → `[1]` in the same diff.
>
> Measured before it shipped (`tools/audit_fire_fuel.gd`, which asks the reaction catalog rather than naming kinds): those twelve cells are the **only painted fire in the repo**, all on DIRT, none with a flammable neighbour — so nothing on any shipped board changes, which is why this landed separately from the spread step. The spread itself is part 2.
>
> **[#890](https://github.com/Phaazoid/Godoiosis/issues/890) part 2 (2026-09-10): fire SPREADS, spent fuel SCORCHES, and the clocks survive a save.** A burning tile takes its flammable CARDINAL neighbours, one step per round (all eight belongs to the ground and is [#891](https://github.com/Phaazoid/Godoiosis/issues/891)'s), and ground whose fuel is spent becomes **`Terrain.TileState.SCORCHED`** -- permanent, harmless to stand on, and **not fuel**. That last clause is the terminator: without it a fire re-crosses what it already burnt and a grass field oscillates for ever, so what it buys is a **FRONT that sweeps and settles**, with the burnt lane behind it the safest ground on the board and the only firebreak that makes itself.
>
> **The refusal is ONE authored clause, not a rule about spreading.** `TerrainReaction.forbidden_tile_state` is `required_tile_state`'s negative and lives inside `applies_to_tile`, so burnt ground refuses a neighbour's flame and a fireball IDENTICALLY -- there is no second answer about what a spread may take, and it is where a firebreak would be spelled if ice ever became one. `admits` is that clause split out for the spread step alone, which has already settled the kind half by holding the cell's fuel reaction.
>
> **The ORDER inside `tick_states` is the rule, and each third of it is its own case:** the take is read **before** the clocks run, so a fire on its last round still passes the flame on rather than stranding the front one cell short of the fuel it was reaching for; the clocks then run down and scorch what they leave; and only **then** is the take lit, each cell on ITS OWN ground's clock, so a fresh catch does not lose a turn on the round it caught. **A cell already alight is never taken** -- two burning neighbours would otherwise restoke each other through `apply()`'s timer reset and no field would ever go out, scorching or not.
>
...
>
> **The tile is a SECOND ATLAS SOURCE** (`Art/Board/IosisTiles.png`, source 1), not an edit to the Solaria sheet the 2D board shares (licensed -- see `docs/asset-provenance.md`) -- and the meshlib generator writes one atlas PER SOURCE, so every existing item id is untouched. Its 3D form costs no new asset: `prop_shape` is #280's **TUFT**, so `BoardMirror` cuts the drawn blades out of the tile and stands one billboard up per blade, at the tile's own `prop_tuft_scale` -- an Objects-tab slider. `move_cost` stays **1**, same as grass: tall grass's cost is fire, and a PER/COH downside arrives with fog of war (dev, 2026-09-10).
>
> **The DENSITY it ships at is [#904](https://github.com/Phaazoid/Godoiosis/issues/904)'s, not this ticket's.** #891 drew five sparse strands standing 0.22 world units against a soldier's 0.50 -- ankle-deep, a LOOK decision taken without a mockup, and the dev rejected it. The shipped tile is ~20 blades over three staggered bands at `prop_tuft_scale` 1.35, and `BoardMirror.tuft_density` is the slider for how many of them get planted. Why regularity rather than blade count was the fault, and why welded blades are a tool: `presentation-effects.md` -> *How much grass a tuft plants is a KNOB*.
>
...
> **[#892](https://github.com/Phaazoid/Godoiosis/issues/892) (2026-09-11): a unit can be FIREPROOF, and the gate is a per-unit layer OVER the ground's own rule.** `Terrain.occupant_damage` still answers what a burning cell charges whoever stands on it; `RulesService.occupant_damage_for(unit, states)` is the layer above it that asks whether THIS unit pays, and both the queue's forecast and the end-of-turn pass ask the outer one. That home is `can_traverse`'s, for #115's reason rather than by analogy: the burn is derived twice (per squad, per faction), so an immunity either derivation owned privately is one the other lies about. **A gate in the executor alone would forecast a burn on a unit that cannot burn** — Law #2.
>
> It keys on **`Terrain.is_burning`**, #174's one spelling, so [#894](https://github.com/Phaazoid/Godoiosis/issues/894)'s fire wall inherits the gate with no edit; a future damaging ground that is not fire gets no immunity by default, which is correct and is what the clause is there to tell its author. The immunity itself is an ordinary granted passive (`Abilities.Id.INSULATED_FIRE`, asked through `Unit.is_immune_to`), so **one question answers a fireball's effect and a burning tile** — first content the **Asbestos Shroud**. Insulation is EFFECT-strip only for a delivered hit (#424) and there is no such half here: a tile burn carries no damage kind and DEF never sees it. See [stats.md](stats.md).
>
...
- **Fire ✦** — damages units crossing; on Wet → **Steam**; expires in ~2 turns unless the tile is **Flammable** — **BUILT, and that expiry rule INVERTED by #890**: how long fire burns is the GROUND's (its ignition reaction's `add_state_turns`), so "~2 turns unless Flammable" became "as long as this ground has fuel, and for ever where there is none to spend".
...
- **Flammable** (object) — catches from adjacent Fire; forests/timber are Flammable children; burns down over turns. **BUILT by #890 as a property of the GROUND rather than an object layer**: a kind is flammable exactly when an ignition reaction keys on it, fire takes its flammable cardinal neighbours a step per round, and spent ground goes SCORCHED and never catches again. The OBJECT form — a burnable barrel, a timber wall — is still [#507](https://github.com/Phaazoid/Godoiosis/issues/507)'s.
...
Two riders either cut must answer. **Destination-only or path-crossing?** — standing in fire and running through fire are different exposures, and the rule should probably not pretend otherwise. And **whose rule is it?** — a preference belongs to the mover (a fire-immune unit should not detour), which points at `can_traverse`'s per-unit layer rather than at the cell. **That hypothetical is real content since [#892](https://github.com/Phaazoid/Godoiosis/issues/892), and the layer it pointed at now has a second tenant** (`RulesService.occupant_damage_for`, the burn's own per-unit gate) — so a pathing preference has a shaped home to join rather than one to invent. Nothing avoids hazards yet, so a fireproof unit needs no pathing work today; the fork below is untouched.
