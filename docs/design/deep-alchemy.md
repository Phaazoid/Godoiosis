...
- `QA`: **Thunder**
  - Generated by lightning reacting through air by the natural reaction `QF + A2 -> QA + FA`
...
- **Copper**: `QFE4`
  - `QE * E2 * FE` (metal + salt + sulfur)
  - corresponding with Venus ♀
- **Iron**: `QF2E3`
  - `QE * FE * FE` (metal + 2 sulfur)
...
- **steel** is an alloy of iron and carbon: `QE*FE*FE + AE`
- **bronze** is an alloy of copper and tin: `QE*E2*FE + QE*E2*WE`
- **pewter** is an alloy of tin and antimony: `QE*E2*WE + Q2E`
- **brass** is an alloy of copper and zinc: `QE*E2*FE + QWE`
...
A fourth erratum surfaced in a later human editing pass and was fixed in the same cleanup: the silver worked example gave the alchemical order of `QE * FE * WE` as `QFWE2` — three E's in that composition, so `QFWE3`. Two notation questions from the audit were then settled in review: element order inside every formula is standardized on `Q < F < A < W < E` (highest to lowest; the stray `EQ`/`EF`/`AF` spellings were inherited from older notes and have been swept), and **Break is commutative** — `Break(F, FE)` = `Break(FE, F)`, same outputs, same natural follow-ups. The commutativity answer sharpens a matching-semantics item rather than closing one: a rune pair selects a *partition*, and distinct partitions of one substance are distinct transmutations — `Break(F2, E)` and `Break(F, FE)` both take sodium (`F2E`) and emit different products (flame + earth vs fire + sulfur). It also means the doc's twin sodium bullets ("source of sulfur" via `Break(FE, F)`, "source of heat" via `Break(F, FE)`) describe one transmutation twice; the genuinely distinct second option on sodium is `Break(F2, E)`.
...
7. **Law #1 — satisfied by the first Bind option, with one nuance each way.** "Breaking down into byproducts in proportion to their component parts" is deterministic as written; just make sure "unpredictable effects" always means *undiscovered*, never *nondeterministic*, and note that "in proportion" implies fractional quantities (see the integer-quantities point below). The second option — refusing Bind on unnamed outputs — collides with doctrine 2 ("every combination does something; no greyed-out slots"), which makes the first option the canon-compatible one.
...
3. **Break's argument order** — resolved in review: commutative. The residue belongs in matching semantics: the rune pair selects a *partition*, and distinct partitions of one substance are distinct transmutations (`Break(F2, E)` vs `Break(F, FE)` on sodium).
