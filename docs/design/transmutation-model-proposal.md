...
>
> **Grilled 2026-07-04:** all six open questions resolved (see *Grill resolutions* at the bottom), and the channeling model rewrote itself along the way — **rune temper + trained-leeway brute force** (see *Temper & channeling*). **The catch-up landed 2026-07-20 ([#60](https://github.com/Phaazoid/Godoiosis/issues/60))** — the in-code channeling gate (`RuneData.temper`/`CIRCLE_CAP`/`CAPACITY`, `TransmutationData.can_channel`) now matches this doc; `RUNE_LEEWAY` is gone. Strain's HP-cost enforcement is deliberately deferred, [#76](https://github.com/Phaazoid/Godoiosis/issues/76) — see *Where this sits* below.
>
...
- **In code:** weighted sigils (`TransmutationData.sigils`, repeats = weight), the flourish lexicon + opposite-rejection (`Classes/alchemy/Flourish.gd`), the slot curve (2n−1), derived-element lookup (Water+Stillness→Ice, Fire+Quickening→Shock) feeding `get_elements()` → combinatrix/terrain. **The doctrine catch-up (#60):** the genetic affinity set (`UnitInstance.affinity`, the Rebecca rule via `has_any_affinity()`), the two-knob rune sizes (`RuneData.CIRCLE_CAP`/`CAPACITY`, `is_legal()` load-time validation), the temper rule (`RuneData.temper`, set permanently by the first inscribed carving, `_fits_temper` enforcing "contains it, never outweighs it"), and `TransmutationData.can_channel` rewritten to floors-=-weight + temper-earned + trained-leeway (`forced_points`/`strain_cost`). `RuneData.RUNE_LEEWAY` is dead.
- **Deliberately NOT wired up (dev call, 2026-07-20):** strain's actual HP cost. The math is real and tested (`forced_points`/`strain_cost` gate `can_channel`'s leeway budget correctly), but the resolver-stage affordability check + payment (audit A7) is a separate, deferred issue — [#76](https://github.com/Phaazoid/Godoiosis/issues/76) — so the channeling *access* rules can be playtested before committing to the HP-cost *consequence*. Today, brute-forcing within budget is free.
- **Not yet code:** the Conjunction identities, repetition thresholds, the codex/discovery layer (recipe→identity mapping, tracked separately as [#75](https://github.com/Phaazoid/Godoiosis/issues/75)), materia offsets, every seed-table entry beyond the two canon derivations, and the maim aura-tax tiebreak's real selector (still enum-order placeholder, [#77](https://github.com/Phaazoid/Godoiosis/issues/77)). Co-dev adjustments land as table/data edits, not rework.
...
**Captured idea (scratchpad sweep, 2026-07-23) — strain as a job-granted verb, not a universal:** brute-force channeling (the trained-leeway-priced-in-strain mechanism) might be something a unit is **granted by a job ability** ("blood transmutation" flavor) rather than a baseline every trained unit owns — without the ability, aura floors are absolute (Rebecca-rule-hard for everyone); with it, the leeway/strain math unlocks. Stays inside the jobs doctrine fence (jobs may work the *strain* side, never grant aura/temper — [job-ideas.md](job-ideas.md) §Watch-list), but ⚠ **would revise ratified doctrine** (trained-leeway-with-strain, co-dev ratified 2026-07-14) — a grill/co-dev item, not a capture-and-build. Natural decision point: [#76](https://github.com/Phaazoid/Godoiosis/issues/76)'s deliberately-deferred playtest of the channeling access rules. Sibling sketch: the **Blood Transmuter** entry in [job-ideas.md](job-ideas.md) §C.
