...
**Captured idea (scratchpad sweep, 2026-09-12) — the DEFENSIVE Quake: a self-root that cannot be moved.** The dev's entry: *"Earth quake transmuation can root opponents, defensive version does no damage, roots self. Can't move, but can't be shoved. Defensive option"*. This lands on a row the table already has — Tier 1's `Earth + Quickening → **Quake** ✦ (tremor line, knockdown/ROOTED)` — and turns it inward: same recipe, no damage, the caster takes ROOTED and buys immunity to displacement with it. Sound in the doctrine's own terms (the shaping marks change how a thing lands, and self-targeting is real in code since [#123](https://github.com/Phaazoid/Godoiosis/issues/123)'s `hits_self`), and it fills a gap the alchemist genuinely has: every carving in this table is something you do *to* the board or to someone else.

⚠ **It asks one question the table cannot answer, and the question is the valuable part: does ROOTED confer shove-immunity, or is that a second state?** ROOTED is defined in [elemental-interactions.md](elemental-interactions.md) as *"can't move next activation"* and says nothing about being moved. If rooting starts to mean *unshovable*, then rooting an ENEMY protects them from a Kinetic Mace, from a Squall, from a cliff edge — the same status becomes a gift, and every existing ROOTED source (Mire, Verdance, mud, PLANT) silently gains a defensive clause it was never authored with. Two ways out, both cheap now and expensive later (design law #4): ROOTED means *cannot move under its own power* and unshovability is its own state that the defensive Quake alone applies; or ROOTED means both and every source above is re-read deliberately. **The expensive version of "can't move or be moved" already exists in this table** — `Stillness ×2 on 2Ae → **Fixation** ✦` — so whichever way this falls, the defensive Quake must stay the cheap, self-only, short echo of it rather than a budget Fixation that reaches an enemy.

**Captured idea (scratchpad sweep, 2026-09-12) — Boulder: a conjured rock that is an obstacle standing still and a weapon rolling.** The dev's entry: *"Boulder creation transmutation. Creates big rock. Normally, rock is non traversable tile (can be destroyed by some drills?) but if used on hill, will roll down hill and big blunt damage"*. The table's nearest existing row is Repetition Axis A's `Earth ×3 → **Bulwark** ✦ — true terrain edit: persistent wall line, rubble cover when broken`; a boulder is that at one cell instead of a line, which suggests it sits well below ×3 and that Bulwark is the plural of it. Three notes for whoever designs it:

- **The "destroyed by some drills" clause is a lane the weapons doc is already holding open.** [weapons.md](weapons.md) retired the Revved-Chainsword-grinds-terrain idea in 2026-08-25 with an explicit rider — *"a melee terrain-attack lane could still arrive on some other verb; it would not be Rev"* — and the Drill is the obvious verb, having already paid for terrain authorship once with Burrow. A boulder that only a Drill can clear gives that lane its first real customer rather than inventing one for it.
- **The roll is the novel half, and Law #2 sets its shape.** A rock that rolls downhill crosses cells the caster did not target and hurts whatever it meets, so the entire roll path has to be resolved and previewed at declare time — the board already has real per-corner heights and ramps ([verticality.md](verticality.md)), so "downhill" is answerable, and `PlanResolver`'s displacement stage is where a multi-cell travel already lives. A roll computed at execution would be the resolver lying about its own preview.
- **It is a terrain EDIT, so it inherits the terrain edits' question, not the attacks'.** The rock is a cell that stops being walkable mid-battle. Cover from Burrow is the precedent for depositing a persistent tile through the normal cell-effect door; what has no precedent is a deposit that takes *passability* away, and whether a unit standing on the target cell blocks the cast, is crushed, or is shoved clear is a fork the entry does not settle.
