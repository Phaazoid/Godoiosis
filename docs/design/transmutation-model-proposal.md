...
Supersedes the flat runestone-leeway point (`RUNE_LEEWAY`, ratified 2026-06-27). Four rules:
...
3. **Brute force = trained leeway, priced in blood.** Your real aura in the temper element is your **leeway budget** for the array's *other* elements — covering breadth (elements you lack) and depth (weight you lack) alike, point for point. 1 fire aura channels 1F+1X and no more; fire-3 can force 1F+1A+1W. Every forced point costs **strain**: recoil HP, superlinear with the forced count (numbers open). **Strain is a COST, not damage (refined at the 2026-07-04 Will grill):** affordability-gated like the Will down-cost — if it would leave the caster at 0 or below they *can't pay*, and the option greys out in battle. It never touches `take_damage` / downing / Crisis ([will-and-death.md](will-and-death.md) → *Transmutation strain*). In-battle affordability greying is NOT the codex — doctrine #2's no-greyed-slots rule governs discovery, not battle affordability. **Carried materia can absorb strain** — fuel substitutes for talent; exact offsets land with the materia pass; other mitigators are open design space. Deterministic and previewed exactly (Law #2).
