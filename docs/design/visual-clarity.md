...
**Canon checked through #1059 (2026-09-20).**
...
**Knobs:** `OverlayManager.DANGER_MODULATE`, `ENEMY_MOVE_MODULATE`, `ENEMY_RANGE_DIM`, `FOCUS_OUTLINE_COLOR`, `ZONE_HIGHLIGHT_MODULATE`, plus `BoardOverlays.outline_width`/`outline_intensity`; each a `CLASS_KNOBS` or `KNOBS` row with a re-apply, and each bright tone re-tints its DIM twin (leave that out and dragging the reach colour strands the crowd on the hue it just left). **Tune the two fills as a PAIR** -- the move tone draws over the reach, so the reach only ever shows as the halo past it, and a move alpha set too high erases that halo entirely.
