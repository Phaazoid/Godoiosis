...
**Canon checked through #1010 (2026-09-17).**
...
## What the enemy COULD do ([#710](https://github.com/Phaazoid/Godoiosis/issues/710) slice 1, BUILT 2026-09-16; reshaped by slice 3, 2026-09-17)
...
**The field is `Classes/ai/ThreatField.gd`, and it is the ARCHETYPE'S OWN ENVELOPE rather than a generic danger zone:** Hold fires from where it stands, a Sentry fires only from its zone + post and only INTO its zone, Rushdown walks its whole move range; each origin is dilated by every fireable attack under the same vertical gate `AITactics._attack_candidates` applies, so the field and the AI's own builder cannot disagree about geometry. Occupancy is read live, which is FE's answer and the honest one for a field the player reads before committing. `game.threat_field()` builds it once and drops it whenever the board moves.
...
**Knobs:** `OverlayManager.DANGER_MODULATE`, `OverlayManager.ENEMY_MOVE_MODULATE`, `OverlayManager.ZONE_HIGHLIGHT_MODULATE`, each a `static var` with a `CLASS_KNOBS` row and a re-apply. **Tune the two fills as a PAIR** -- the move tone draws over the reach, so the reach only ever shows as the halo past it, and a move alpha set too high erases that halo entirely.
