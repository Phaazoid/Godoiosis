...
3. **The Transmutation submenu should read as a catalogue, not just a picker** ([#88](https://github.com/Phaazoid/Godoiosis/issues/88)) — **now tracked as [#166](https://github.com/Phaazoid/Godoiosis/issues/166)** (filed 2026-08-10 out of [#157](https://github.com/Phaazoid/Godoiosis/issues/157)'s plan, a #44 sub-issue; it lived only in this doc line until then). #157's equip gate killed the fully-dead-rune case but is "at least one channelable carving", so a partial rune's dead entries permanently owe the player a reason — the gate makes this readout more necessary, not less. Two parts, both dev-requested at build time:
