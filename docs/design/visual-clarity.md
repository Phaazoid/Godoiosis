...
## The enemy's reach, before you commit ([#710](https://github.com/Phaazoid/Godoiosis/issues/710) slice 1, BUILT 2026-09-16)

**The ask (dev, 2026-09-02):** *"before you've executed your plan, the game should preview which of your units the AI will attack"* -- Triangle Strategy's lines from each enemy to its target, Fire Emblem's danger zone. **And (2026-09-16): let the player see the zone a patrolling squad is leashed to.** It is the payoff of the predictability law in [ai-tactics.md](ai-tactics.md): the preview is what lets the AI get less obvious later without getting less predictable.

**Two tiers, because the two questions cost different things -- measured on Castle Assault at HEAD, one enemy squad of five.** *Who CAN reach this cell* is a reach field (`compute_move_range` x `Reach.get_all_attack_cells_from` for every enemy): **~6 ms for the whole board.** *Who WILL attack whom* is the AI's real decision: **~300 ms per squad** (`queue_group_move` 69, the joint attack pass 155 -- ~25 full plan resolutions at 3 ms each plus a 22 ms `queue_action` per round -- the fallback verbs 34), and it is a SEARCH whose every input is a projected cell, so nothing caches across a pointer sweep and no restructure gets it under a frame. The dev's ideal was the exact plan on every hover; the number said no, and the split is his ruling: **the hover tier is live and the plan tier recomputes on every plan change and on the toggle.** A silent planning path (skip `queue_action`'s validate + redraw) is a real 2-3x and is filed, not built.

**The hover tier (this slice) is `Classes/ai/ThreatField.gd`, and it is the ARCHETYPE'S OWN ENVELOPE rather than a generic danger zone:** Hold fires from where it stands, a Sentry fires only from its zone + post and only INTO its zone (the lure-proofing contract, read as a superset), Rushdown walks its whole move range; each origin is dilated by every fireable attack under the same vertical gate `AITactics._attack_candidates` applies, so the field and the AI's own builder cannot disagree about geometry. Occupancy is read live, which is FE's answer and the honest one for a field the player reads before committing. `game.threat_field()` builds it once and drops it whenever the board moves -- a queued or cancelled order, a turn start, a spawn, a board load, the end of a playback pass -- so a hover is a dictionary read. **The wire is pinned:** `tests/ai/test_threat_field.gd` builds the field, runs the real archetypes through the real `SquadManager`, and asserts every attack they queue aims inside it.

**Three marks, all on the GROUND channel (#346: the tile is what the interaction is about), and all copied 2D -> 3D by `OverlayMirror` the way every other layer is:**

- **Threat lines** -- one straight line from every enemy that could reach the destination under the pointer in `CHOOSING_MOVE` / `CHOOSING_GROUP_MOVE`. Stored as data on `OverlayManager` (`threat_lines` + a version, the sight trace's shape), drawn flat by `ThreatLines2D`, lifted through `BoardSpace.trace_point`, gone with the mode. `BoardOverlays.set_lines` grew for it: one pooled `ImmediateMesh`, one surface per segment.
- **The danger fill** -- a `DangerOverlay` layer UNDER the move fill, so a reachable cell still reads over a threatened one. Hovering an enemy in IDLE shows that enemy's own reach; **T** (`toggle_threat_view`, sticky, both views) shows the whole field and keeps it up while you plan.
- **The leash reveal** -- a Sentry's PATROL zone on the picked-zone highlight layer, while one of its members is hovered or T is held. **This reverses [missions.md](missions.md)'s 2026-09-11 "PATROL is authoring-only" rule, on demand only**: the patrol LAYER stays behind the Tile Brush gate (`OverlayMirror` mirrors `ZONE_PATROL` on `zones_authoring_visible` alone), and the highlight's gate became *authoring OR revealed*. Reusing that layer rather than minting a fourth zone surface is the Law #4 call #955 will want made once: the brush's pick and a play-time reveal are the same question, *this zone, singled out*. The reveal stands down while the brush tab is up, so the layer never has two writers at once.
