...
lands during `Pacing.AI_PLAN_READ`, the beat that exists so a drawn AI plan can be read) was filed
as [#362](https://github.com/Phaazoid/Godoiosis/issues/362) and **RULED A FEATURE on 2026-09-16**:
the dev wants *more* enemy intent shown *earlier*, which is [#710](https://github.com/Phaazoid/Godoiosis/issues/710)
(see *The enemy's reach, before you commit* below).
...
**The ask (dev, 2026-09-02):** *"before you've executed your plan, the game should preview which of your units the AI will attack"* -- Triangle Strategy's lines from each enemy to its target, Fire Emblem's danger zone underneath.
...
**It carries TWO envelopes since slice 3, and the second was already being computed and thrown away.** `_origins_of` answers where a unit may fire FROM; keeping that as `move_by_unit` beside `by_unit` gives the FE two-tone for about fifteen lines. **Archetype-honest by construction rather than by a second rule** -- a Hold unit yields exactly its own cell, because that is where its archetype fires from. Under a raw move range it would paint its whole MOV and read as a charge that is never coming, which is the dev's ruling and the reason it is not FE's version of the same picture.
...
**The board draws it as two fills and a leash, all on the GROUND channel (#346), all copied 2D -> 3D by `OverlayMirror`:**

- **The reach tone** (`DangerOverlay`, red) -- every cell an enemy could attack.
- **The move tone** (`EnemyMoveOverlay`, blue) -- every cell it could stand on, drawn **OVER** the reach (dev ruling: a body's standing room is the louder fact) and **UNDER** your own yellow move range, so the cell you are about to step on still reads. That ordering is the whole reason the negative sort stack moved down two in slice 3: `DANGER` shared `MOVE`'s sort 0, and with equal `render_priority` and equal depth the winner fell out of pool allocation order rather than out of the table. In 2D it is tree order; in 3D it is the sort number, and `test_your_own_two_tones_draw_over_the_enemys_field` pins the relationship (renamed with the ruling it pins -- see the repeal above).
- **The leash reveal** -- a Sentry's PATROL zone on the picked-zone highlight layer. **This reverses [missions.md](missions.md)'s 2026-09-11 "PATROL is authoring-only" ruling, on demand only.**
...
**The reach LINES are GONE** (dev, 2026-09-17: *"it's functional, but it's a bit too much"*). A beam per attacker answered the same question the two-tone fill answers for the whole board at once. `Layer.THREAT_LINES`, `OverlayManager.show_threat_lines` and `ThreatLines2D.segments` went with them.

> **REPEALED by [#1066](https://github.com/Phaazoid/Godoiosis/issues/1066) (2026-09-21), by the same dev playing it back: _"if multiple are highlighted, and you hover one, the other ranges become too dim. They shouldn't dim at all, the outline on the main one should be the only differentiator."_** `DANGER_DIM`, `ENEMY_MOVE_DIM`, `ENEMY_RANGE_DIM`, `dimmed_tone` and `show_dim_ranges` are DELETED rather than neutralised, so a stale caller is a parse error. The stroke below carries the whole job alone. The three decisions recorded here are kept as the record of *why a dim tier is hard to get right* — the subtraction in particular is a real hazard the moment anybody reaches for one again.

**...AND THE CROWD DIMS SO ONE FIELD CAN BE READ OUT OF IT (slice 4, dev: _"when hovering a specific unit, their grid should highlight"_).** With V on, three enemies union into one blob and the pointer changed nothing about it. Every enemy the pointer is NOT on now falls back to `DANGER_DIM`/`ENEMY_MOVE_DIM` -- the same two tones at `ENEMY_RANGE_DIM` alpha, so the crowd still reads as move-versus-reach, just quietly -- while the hovered one stays at full strength. Three things are decisions rather than mechanics:

- **The crowd is subtracted from the focus and never the other way.** The bright pair goes on overlapping exactly as it shipped, because the move-over-reach ruling composites blue OVER red and making those two disjoint would repaint every cell the dev approved. What must not happen is a dim quad UNDER a bright one: four coincident alphas read differently from two, so a focused cell would change tone wherever a neighbour's field crossed it.
- **With nothing hovered the dim layers are UNUSED, not merely equal**, so a board nobody is pointing at looks exactly as it did.
- **The dim is ALPHA ONLY.** Multiplying the whole `Color` the way `BLOCKED_REACH_DIM` does pulls both hues toward the dark board until neither reads as a hue -- drawn both ways in the mockup before choosing.

**A STROKE ROUND THE FOCUSED FIELD** (`Layer.ENEMY_FOCUS_EDGE`, the dev's addition to that mockup: _"a small outline to the range of the hovered unit, so that when multiple units are in a group, it is very clear which field you are hovering"_). One two-point segment per outward-facing cell edge of the whole footprint -- move and reach together, since what it answers is *this is the field you are pointing at* and not which tone a cell carries. It needed no new machinery: `ThreatLines2D.segment`'s TRACE SPACE is already the shared convention, so the flat view flattens it and `OverlayMirror` lifts it through `BoardSpace.trace_point` exactly as it lifts an intent beam, and each endpoint takes the inside cell's own corner height so the stroke follows a ramp. It DID need two things stated out loud: for a LINE layer `sort` has only ever meant render priority (`set_lines` applies no lift), so the spec names a separate `lift_sort` -- the first above every FILL -- and it carries its own beam width and brightness, because the shared trio is tuned for a laser and at the sight bead's width and bloom a stroke round forty cells reads as a rope of light.

### ...and what #1066 re-cut about the VOCABULARY (2026-09-21)

**The ask came back as a whole-readout complaint, not a range one** (dev, 2026-09-20, playing #1042/#1059): *"I think we need some changes, still. Not just to the threat lines, but to the targeting as a whole."* Four items, and then — mid-thread, in place of answering four options put to him — a replacement of the entire colour vocabulary:

> Actually, let's go with what fire emblem does. Enemy range is a reddish purple for the whole thing, movement and attack range, unbroken. Player range is what we currently have for enemies. When enemy range overlaps with your blue, it turns it a bit purplish to show that there is intersect.

**Three layers now, and the renames are the point.** `Layer.DANGER` would have meant *the player's reach* and `Layer.ENEMY_MOVE` would have meant nothing at all, so:

| today | what it is |
|---|---|
| `Layer.MOVE` (sort 0, **blue**) | where your hovered or selected unit may STAND |
| `Layer.REACH` (sort −1, red) | where it could HIT from anywhere in that blue |
| `Layer.THREAT` (sort −2, reddish purple) | the enemy's WHOLE field, move and reach together |

**THE ORDER IS THE DESIGN, and it is the reverse of slice 3's.** Yours sorts ABOVE theirs, so a cell both fields cover composites as blue TINTED purple rather than as purple — which is what says *both* with no third colour authored anywhere. That composite is what the dev judged: four purples were drawn, each in three places (alone, over your blue, beside your red), and he picked **C, "barely purple"**, with *"could perhaps be a little darker"*. A sort IS a plane (`_lift_of`), so this is a real depth relationship and not pool-allocation luck; `test_your_own_two_tones_draw_over_the_enemys_field` pins it, and a sibling law refuses the four deleted layer names outright.

**Sorts −3 and −4 fell vacant with the dim pair and are LEFT vacant.** Compacting them would move the whole negative stack, and what a shift threatens is the FLOOR — `_lift_of` is `fill_lift + sort * lift_step`, and the zone band at −7 sits at 0.012 with `fill_lift` 0.04.

**The one thing here that is not a colour: the player's reach may not ride `ThreatField`'s own origins.** `_origins_of` unions `reachable` with `squad_unreachable` on purpose (see *COHESION IS IGNORED* above), which is right for a **prediction** about an enemy and wrong for a **permission** about your own unit — your blue is where you may actually be ordered, so your red must grow from those cells and no others. So the walk is exposed rather than reused whole: **`ThreatField.reach_from(unit, board, origins)`**, a thin public static over the private `_reach_of` the enemy path already calls, with the origins passed IN (Law #4's *pass it in, don't rebuild it locally*). The enemy path is untouched and the counter rim and archetype rules come along for free. **The rule at every caller is that the red grows from exactly the cells drawn BLUE plus the one the body is standing on** — `get_move_range` drops that cell because there is nothing to say about walking where you already are, and everything to say about shooting from there — which is also what makes group-move mode correct for free: its blue is only the FOLLOWABLE subset, and the red narrows with it.

**Cost, reasoned rather than profiled:** one reach walk per hover change over ~40 origins, against `docs/performance.md`'s measured 1.5 ms for `ThreatField.build` over *every enemy on Castle Assault*, cached until the board moves. If hovering ever feels sticky, re-measure before believing this.

**A PINNED ENEMY FLASHES** (dev: *"units that are toggled need to be indicated in some way. I think they should flash, too."*), and the surface is his ruling too — the unit SPRITE, not a marker, which gets the diorama for free through `UnitMirror`'s per-frame `modulate` copy. It is a THIRD tenant on that channel beside the aim pulse and the hover highlight, so the precedence is stated in code rather than discovered: **the aim pulse outranks it**, because *this unit is about to be hit* is news where *you pinned it* is a bookmark you set yourself. `UnitVisuals.pinned` is a flag rather than a reading of the tween precisely because the pin OUTLIVES its own pulse and has to come back when the aim moves on — the half a plain precedence misses, and its own case. `game._sync_pin_flashes` sweeps every unit on each redraw rather than diffing: `set_pinned` is idempotent, and a pin ends three ways (shift+click, the key's OFF, the body dying), so one sweep is cheaper than three correct diffs.

**THE GROUND FILLS CAME UP** (dev: *"the alpha values in the ground tile overlays are a little too light, we want to make them a bit less see-through"*), across the three range fills only. The orange cohesion bubble and the mauve `INVALID_MOVE` are deliberately untouched — see the residuals below.

**Knobs:** `OverlayManager.REACH_MODULATE`, `THREAT_MODULATE`, `FOCUS_OUTLINE_COLOR`, `ZONE_HIGHLIGHT_MODULATE`, `BoardOverlays.LAYERS[MOVE]`'s colour, plus `outline_width`/`outline_intensity`; each a `CLASS_KNOBS` or `KNOBS` row with a re-apply. **Tune them as a STACK, never one at a time** — what the player reads is what they composite to where they cross, and there is no fourth colour authored for that intersect anywhere. Note the standing fork the `MOVE` row inherits: a `layer` knob moves the 3D entry, and `OverlayManager._ready` restates the same literal for the 2D, exactly as the zone layers do.

**Declared residuals at close:**

- **The intent cone is still a see-through triangle** and the dev wants *"a solid 3D shape around the line"*. Recorded on PR #1061 with the mechanism sketched; its own pass.
- **The orange cohesion bubble and the mauve `INVALID_MOVE`** both sit beside the new blue and the new purple, and `INVALID_MOVE` is itself a desaturated reddish purple. Ruled *re-evaluate after this lands* (dev, 2026-09-20).
- **What the suite cannot see, and is the dev's:** whether the darkened purple reads against the red at the new alphas, whether the intersect says *both* without a third colour, and whether a board carrying three enemies plus your own two tones is legible.

## ...and what they WILL do ([#710](https://github.com/Phaazoid/Godoiosis/issues/710) slice 2, BUILT 2026-09-17)

The other half of the ask: not *who could reach this cell* but **who will attack whom, and for how much**. Each hostile squad's real decision is run against the board the player's pending plan will leave behind.

### The mechanism is a POSITIONAL SNAPSHOT, and that is the whole design

The preview has to answer *where will everyone be*, and the first draft answered it three times: fix the whiff gate, give published knockback an owner, teach the AI's reads to project. Each was a third of the fix -- `AITactics` picks targets and walks from the player unit's LIVE cell at eight sites, and `compute_move_range` starts a shoved enemy from where it stood.

**So the preview moves the units instead.** `AIController.preview_turn` stands every unit on its `get_projected_destination()` for the duration, plans, harvests what they intend, and puts them back. Every read -- movement, targeting, the whiff gate -- is answered correctly with no rules code edited. Safe on three facts that were checked rather than assumed: planning is wholly synchronous, `UnitMirror` reads `position` in `_process`, and `set_cell` is already the teleport door.

The generalisable shape: **when several readers disagree about a fact, moving the fact beats fixing the readers.**

### What it costs, and the early-out slice 1 paid for

A recompute skips any squad whose reach envelope holds no hostile unit -- exact, because that field is a superset of what the squad's own builder could aim at. **The early-out weakens as ranges grow** (dev's caveat): a long-range unit is legitimately never skipped.

**`is_squad_actable` split for this.** It refuses a squad whose `has_acted` is true, and that flag is cleared only for the *incoming* faction at its own turn start -- so during the player's turn every enemy squad still carries last turn's, and asking it would have returned an EMPTY preview from turn 2 onward. Under the always-attack ruling an empty preview reads as *provably safe*: a confident silent lie, on every turn but the first. `is_squad_previewable` is the same predicate minus that clause, which is correct rather than a loophole -- the flag is about the turn that finished, and the preview is about the turn that is coming.
...
He played it and reported three things: *"They should be shallow arcs rather than straight lines, and the point of origin needs to come from the actual unit rather than over its head. The arrows also just don't look great in practice. Perhaps a narrow cone at the end instead?"*

**The origin complaint is exact, and the numbers are the whole finding.** In world units above a unit's feet: the median character's head reaches **0.625** (`MapSpriteInk.INK_RECT` is 20 texels at `UnitSprite3D.texels_per_unit` 32), the mark hung at **0.5** (`Reach.EYE_HEIGHT` 1.0 x `BoardSpace.ROW_HEIGHT` 0.5), and the crown hangs at **0.85**. So the mark left at **80% of body height** -- and `Reach.gd`'s own header records the dev's 2026-08-20 ruling that it should be *"the center of the sprite"*, which is 0.3125. It had never been that.

**`Reach.EYE_HEIGHT` CANNOT MOVE FOR A LOOK, so the mark gets its own height -- a DECLARED second answer (Law #4).** That constant defines what a wall is for `Reach.sight_trace` and the vertical aim gate; lowering it would change which shots are legal. The declaration: **a sight trace draws the TRAJECTORY the rule judges and must hang where the rule says; an intent mark is a statement about who attacks whom and may hang where the body is.** `ThreatLines2D.MARK_HEIGHT` is that answer, in rule units, beside the mark's other shape statics.

**...and that decision uncovered a latent bug, which is the half worth remembering.** `segment()` measured from `board.elevation_at`, a cell's **LOW side** -- `BoardHeights` says so, and for a ramp it is the floor the ramp climbs FROM -- while a body on that ramp stands at the slope's **visual midpoint**, half a level higher (`verticality.md`, and the reason `BoardSpace.surface_point` exists). `EYE_HEIGHT`'s 1.0 rule unit happens to EQUAL that half level, so a ramp mark left at the feet **by coincidence**; at the sprite's centre it would have sunk 0.19 world units into the slope on every ramp cell in the game. It reads `Terrain.height_at_uv(corners, 0.5, 0.5)` now -- #427's one surface -- which agrees with `elevation_at` on flat ground, so flat boards are untouched by construction. **A constant that is right for the wrong reason survives every test until something else moves.**

**The bow is authored PER CELL of run, never as an absolute.** "Shallow" is a property of the shape rather than of a distance: a fixed lift is a hump on an adjacent-cell mark and nearly flat on a nine-cell one, so one number could not read as shallow at both. `MARK_BOW_PER_CELL` is measured off the card the dev ruled on.

**The cone rides `UV2.y` as `scale - 1.0`, and that offset encoding is the design.** A ribbon's width is a per-MATERIAL uniform, so a shaft and a cone on one layer cannot differ by one; `UV2.y` was the only free per-vertex channel (`add_beam_strip` wrote `Vector2(travelled, 0.0)`). Carrying `scale - 1.0` rather than `scale` makes **zero mean unchanged**, so the sight bead and `ArcLightning`'s bolts are untouched *by construction* rather than by care -- the same inert-by-default discipline #1042 gave the bead and flash uniforms. A mark is two strokes (the sampled arc, then the cone) and they SHARE the base point, so the bead's chained measure stays continuous with nothing double-counted.

**Declared residual:** a proportional bow means a long mark rises high -- a nine-cell one lifts ~1.1 world units, above the crown band. Left uncapped rather than clamped, because a cap is a second rule to tune; the per-cell knob is the first thing to move if it reads badly across a big board.
...
**STICKY across turns, booting at INTENTS** (both dev calls). Turning it off should buy more than one turn of quiet; and a stranger who never finds the key should still see the feature, which is what the demo bar is for. **The enemy RANGES are deliberately NOT in this cycle** -- they are V's, and they answer what an enemy COULD do rather than what it WILL.

**EVERYTHING adds the damage, drawn as the predicted span on the victim's own health bar** (dev: *"instead of showing a giant number, I'd like to show it how we already do"*). That makes `UnitMirror`'s prediction channel the first with TWO writers, and they **compose rather than one winning**, because that is the real timeline: your plan resolves, then they act.

**The arithmetic needs no rules call.** `ThreatIntent.damage` is already `hp_before` minus the DISPLAYED hp after, and `LethalityRules.displayed_hp` answers 1 for a down and 0 for a kill -- so subtracting it from a unit your plan does not touch lands exactly on the rung. Summed **per victim** rather than kept per attacker, since a bar draws one span; which attacker owns which part of the bite is [#1012](https://github.com/Phaazoid/Godoiosis/issues/1012).

**Two limits, both over-warning:** an intent was measured against the board BEFORE your plan, so a heal plus a lethal blow shows the alarm over a bar that is not empty; and two non-lethal bites summing past a unit's HP clamp at zero and alarm, because this readout cannot tell a down from a kill without running the ladder.
