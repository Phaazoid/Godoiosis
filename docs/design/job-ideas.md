...
1. **Vanguard** *(team · —)* — the shield of the line. Starter: **Taunt** [R] (standing policy: counters against my party must target me — the C3 override). Main: ~~**Guard** [R]~~ **SHIPPED PAST THIS ENTRY ([standing-reactions.md](standing-reactions.md), [#414](https://github.com/Phaazoid/Godoiosis/issues/414), 2026-08-21): Guard is a BASIC main action everyone has, not a job ability — so the Vanguard's version of it is its **brace bonus**, i.e. carrying `Abilities.Id.BRACE` in its pool. Note the sub-tier name below is now TAKEN: the shipped Brace is +DEF on a hit you absorb, not +MHP — rename this sketch's before authoring it.** **Immovable** [P] (cannot be shoved — Weight override; anti-Bludgeon/air-burst teeth). Sub: ~~**Brace** [P] (+MHP)~~ — see above. Ceiling: DEX ≤ 4 (plate is free). MOV 3.
...
- **Brute-force channeling as a granted verb** *(scratchpad 2026-07-23)* — the strain/leeway system *itself* might be job-granted (**Blood Transmuter** — blood-transmutation flavor: pay flesh for what talent won't cover) instead of universal-for-trained: without the ability, aura floors are absolute for everyone; with it, the trained-leeway/strain math unlocks. Inside fence 1 (works the strain side, grants no aura/temper) — but ⚠ **revises ratified doctrine** (trained-leeway universal, co-dev 2026-07-14): grill before authoring. Canonical capture + decision point ([#76](https://github.com/Phaazoid/Godoiosis/issues/76)'s playtest): [transmutation-model-proposal.md](transmutation-model-proposal.md). Kinship: Vessel/Adept already work this side.
