...
**The card**: the unit fills the left (sprite, name, limbs, job — absent when the mission offers none and the unit holds none, #964 — live abilities, the eight effective stats), what it *carries* fills the right as the same row the stash uses — six of them, because that is `MAX_INVENTORY_SIZE` — and the strip along the bottom is weight and effective DEF plus the deploy toggle. Every number is read, never re-derived: `get_effective_stat`, `get_effective_def`, `get_weight`, `get_live_abilities`, which is exactly what ruling 7 spawns the whole roster as real `Unit`s to make possible. **A card whose unit is going wears the friendly ink around it** (`READOUT_ALLY`, dev 2026-09-05, from his original mockup) — on the **border**, never the fill, because the fill is the ground every chip and number on that card is read against; the content margin is held across both states so a card does not resize the instant it deploys. `game.is_deployed` is the one answer behind it: `units_root` membership is what "deployed" means, and the outline, the toggle's own label and the screen's toggle handler all ask it there rather than spelling it three times. **The toggle itself wears the same idea one control down** (#978, dev 2026-09-16, from a mockup): an outline that is `READOUT_ALLY` when the unit is coming, `READOUT_BENCHED` when it is staying and could be placed, and chrome grey when it cannot be placed at all. That third state is the button's own `disabled`, which is why it covers **both** of `_deploy_block_reason`'s answers — a full cap and a full deployment zone — without anyone asking a second question. Border-only for the card's reason; the ink follows the border except on the dead state, where a chrome grey as *text* falls under the contrast floor and a muted text role carries it instead.
...
**AND THE REGION IT LEAVES BEHIND IS A ROW BUDGET, NOT JUST A SIZE** (#977). `BAND_HEIGHT` is what the bottom band takes; what is left after the screen's margins and the region chrome is the card grid's *viewport*, and a viewport that lands mid-card clips a row. At 210 it landed 2 px into the second row — exactly the width of a deployed card's outline, so the row read as drawn wrong rather than as scrolled past. The constraint is that the viewport must seat whole rows and stop **before** the next one starts, i.e. land inside the gap the grid's `v_separation` leaves; the value is chosen for the middle of that window rather than flush against an edge, because `CARD_HEIGHT` is a *minimum* the card's own content has already grown past once. Sibling of the flag rule above in the way that matters: the scrolling case cannot see it, since it asks whether a list is bounded and how wide it came out and never where the clip falls — so `test_the_roster_clip_never_cuts_a_card_row` is where the constraint is a law rather than a comment.
...
**A THIRD member of that family, and the one where the CONTAINER answers rather than the box** ([#989](https://github.com/Phaazoid/Godoiosis/issues/989), 2026-09-16, reported as Ross's *Iron Will* being cut off). A `clip_text` Label declares a minimum width of **1** — that is what the flag means — and a container lays a child out at its *combined minimum*, so a chip carrying `custom_minimum_size.x = CHIP_MIN_W` was drawn at 30px whatever was written in it, with the whole `meta` column sitting unspent beside it. The card's law is that nothing may demand width from its content, and the 30 was keeping it by refusing to draw rather than by bounding: a chip asks for its own measured text now, capped at **the same constant the job picker demands of that column**, so the card's minimum size has not moved by a pixel and the cap is arithmetic rather than taste. (Stated as a shared ceiling rather than as *the picker already demands it*: since [#964](https://github.com/Phaazoid/Godoiosis/issues/964) a mission that offers no job draws no picker, and a column without one is narrower than a column with it — never wider — so the bound holds either way while the sentence that justified it would not have.) Why only the abilities row showed it: one helper, two vocabularies — the limb chips are two-letter codes that fit 30px, and the 30 was sized at that caller. **A shared helper's constant is a fact about whichever caller it was measured at**, and the next caller inherits it silently.

**A FOURTH member, the first one OFF this screen, and the reason the measuring finally moved** ([#1024](https://github.com/Phaazoid/Godoiosis/issues/1024), 2026-09-18, reported from play an hour after [#1022](https://github.com/Phaazoid/Godoiosis/issues/1022) merged). #1022 took `SIZE_EXPAND` off the rune card's carving name, which is what stopped the cost bars being flung to the far edge — and nothing replaced the width, so an `HBoxContainer` gave the label the one pixel a clip declares and every carving name in the game shipped as a sliver of its first glyph. **#989's fix was right and stayed private to `PreMissionCard._chip`**, so the second card could not inherit it; `UiText.fit_width(label, floor, ceiling)` is the measuring now, and **the bounds stay at each caller**, which is the paragraph above applied rather than restated. The rune card's ceiling is half of its `CARD_W`: the bars are guaranteed the other half whatever goes into a `display_name`, and `CARD_W` is a *minimum* on the card's content rather than a cap, so an unbounded name would widen the modal.

**The law has TWO halves and only one of them is fixture-free**, which is what `tests/support/label_fit.gd` exists to say out loud. The STRUCTURAL half — *a clipped label under a row-laying container carrying neither `SIZE_EXPAND` nor a width of its own* — is the bug itself, and it cannot be dodged by picking short fixture text; the CONTENT half (`draws_in_full`, which had pinned this rule at three surfaces since #944 and could not travel to a fourth) is what stops an answer of `1` counting as having asked. Only HBox and HFlow are in the family: a VBox, a MarginContainer or a PanelContainer stretches a child to its own width, so a clipped label in one of those is bounded by its column and clipping there is the design — `ModFittingCard`'s mod descriptions run past their column on purpose, with the sentence on the row's hover, and a `draws_in_full` sweep over that card would assert the opposite of what it is for.
...
**The JOBS flag defaults TRUE where the other three default false** ([#964](https://github.com/Phaazoid/Godoiosis/issues/964)), and the asymmetry is forced rather than chosen: the first three lists were authored *before* their flags existed, so false preserved what those files already said, while no roster has ever named a job — false would have meant the plumbing diff itself switching every picker off, rather than the day somebody curates a list. What a default preserves depends on what is already in the files, not on what its siblings do.

**`Roster.offered_entries()` / `offered_stash()` / `offered_mods()` / `offered_jobs()` are where that resolves**, one accessor per list, so no reader ever asks about a flag. **`offered_entries()` SYNTHESIZES entries under its flag, and that made resolution-once a correctness rule**: `deploy_roster` runs two walks and the second keys `unit_of_entry` by the entry, so asking the accessor once per walk hands them two different sets of objects and nobody stands up. A mutant proved no existing case could see it — the flag is off in every authored roster, and with it off the accessor returns the same array twice.

**The mod pool rides `Loadout`** — and the job offer with it since #964, through the same door and for the same reason — the one phase object that outlives the draw: `deploy_roster` resolves the `Roster` and drops it, so by the time a card opens nothing else is holding the answer. What the job offer does NOT copy is the last hop: `WeaponModCatalog.offerable_for` reads an **empty pool as every mod**, so *empty means none* is true of the roster field and not of the fitting card that reads it; `PreMissionCard` means none at every layer. Mods ride it as shared refs, the stash as copies — a stash item is owned by whoever holds it and moves between owners; a mod is a shared definition a weapon points at.
