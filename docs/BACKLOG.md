...
**2026-08-26 — [#412](https://github.com/Phaazoid/Godoiosis/issues/412) queue order becomes real + [#413](https://github.com/Phaazoid/Godoiosis/issues/413) Overwatch, one branch:** moves now resolve in the order they sit in the queue (every mover seeded at its ORIGIN before any walks — that seeding is what makes the ordering real rather than a list order), the queue panel's drag reaches every stored-order section instead of attacks alone, and a carbine can stand watch: aim an attack, hold fire, and the first active enemy to ENTER a watched cell takes the shot. Canon + the build's own rulings: [`docs/design/standing-reactions.md`](design/standing-reactions.md). Ships with placeholder mark art and the shots playing back after the move phase rather than interrupting the walk (both declared, both filed).
...
- **Squad-tether range fix** — `compute_move_range` now clamps members to the leader's squad range (`get_max_squad_range` — the leader's `COH` stat since #142; a static `SQUAD_RANGE` const from #63; LDR-derived when this landed), consistent with `validate_squad_plan`.
