...
- Cover should POP UP in 3D as three individual mud bumps, not the whole sprite rotating vertical → **issue [#326](https://github.com/Phaazoid/Godoiosis/issues/326)** (the dev re-raised it as *"burrow terrain should pop like the flowers do"*, which names the answer: #280's `PropShape.TUFT` already keys out the background, splits the art into 8-connected clusters and stands each one up at its own place — exactly "three individual bumps". His 2026-08-14 doubt *"I worry this might not be as easy as a simple transform"* was correct and is now moot. The part that does NOT transfer: cover is a runtime terrain STATE on `Layer.TERRAIN`, not a tileset tile with a baked `prop_shape`, so the mechanism carries and the plumbing doesn't) (2026-08-16)
