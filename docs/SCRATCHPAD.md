...
- Volley attacks grouped into one expandable action-queue row → #44 Visual Clarity umbrella (2026-06-26)
...
- Click-drag to reorder attacks in the action queue → #44 (2026-06-26)
- More action-queue info per row: damage on hit + target HP before→after (`ResolvedOutcome.target_hp_after` already threaded) → #44 (2026-06-26)
...
- Action queue needs an outer scrollbox — the *list of action-type sections* can overflow, not just each section → #44 (2026-06-26)
