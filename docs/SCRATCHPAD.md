...
- (ui/accessibility) **Let the PLAYER pick their own aim/attack reticule colours.** Dev's aside while approving #212 slice 2, half-joking: *"Maybe in the final game, we can even give the player a color slider to set whatever color they want for their aim/attack reticules, ha."* Worth keeping because it is closer than it sounds — slice 2 makes the reach fill a tunable `OverlayManager.ATTACK_MODULATE` rather than a `const`, which is exactly the seam a settings screen would drive, and the same 2D-modulate path already feeds the 3D. *(recorded 2026-08-14 — cross-refs: real colour-blindness support would want the whole `LAYERS` / `OverlayManager` modulate set, not just the reticule, so this points at a player-settings surface the game has none of yet; sibling of #217's photosensitivity toggle, which is the other accessibility knob with no home.)*


- (dev tools / look) **The glow settings belong on the Fire tab.** Move the `Post` group's glow knobs out of the general post-processing rows and onto the fire tab, where they are actually being tuned. *(recorded 2026-08-18 — premise check: the glow rows are `LookKnobs.gd:74-83` (`WorldEnvironment.environment:glow_*`), i.e. the LookTool `Post` group; the `Fire` group lives in `ObjectKnobs.gd` (#272's Objects tab). The two tables have different DESTINATIONS — `LookKnobs` writes a live mood value the mission/preset wears, `ObjectKnobs` writes an `@export` default — so this is a placement question, not a straight row move. Glow is also global, not per-fire: whatever moves has to stay a scene-wide knob even when it is shown beside the flame it exists for.)*

- (ui) **Warn on the action menu when a move ends on an element-afflicted tile.** If a character is about to move onto a tile carrying an element (fire, and whatever else lands later), the action menu should say so before the player commits. *(recorded 2026-08-18 — captured, not locked: the open half is what the notice IS — a line on the menu entry, an icon, a confirm — and whether it covers only the DESTINATION tile or any afflicted tile the path crosses.)*
