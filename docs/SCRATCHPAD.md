...
- (presentation) **Cover should POP UP in 3D the way units do — but as three individual mud bumps cropping up, NOT the whole sprite rotating vertical.** Dev's words, found while feel-checking #231 PR 2: *"I worry this might not be as easy as a simple transform."* He is right that it isn't — units get their pop from a billboarded sprite standing up, and applying that to cover would stand the entire cover sprite on its edge, which reads as a wall rather than as lumps of ground. Three discrete bumps means real geometry (or three small sprites placed per-bump), i.e. an ART task with a mirror change behind it, not a transform tweak on the existing marker. *(recorded 2026-08-14 — explicitly NOT for #231; cross-refs: cover currently rides `OverlayMirror._terrain`'s `Layer.TERRAIN`, a flat ground QUAD anchored at `TOP`, same path as the FROZEN icon. Related to #218's verticality work, since anything that stands up off the top face has to answer to elevation.)*
