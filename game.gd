...
func _input(event: InputEvent) -> void:
	...
	# before reaching anything, and the 3D view is the only one a player ever sees.
	#
	# EXACT_MATCH, which no other check on this page asks for. `is_action_pressed(action, allow_echo,
	# exact_match)` IGNORES additional modifiers by default, so a Shift+F3 press matches the plain-F3
	# binding too -- and Shift+F3 is the dev's note-less instant report (DevController). Without the
	# third argument one keypress fires both: the card opens AND a report files behind it.
	#
	# Deliberately NOT gated on _board_locked_for_player(): a pass playing back at you is exactly
	# when something looks wrong, and the pause menu is already reachable there (#723).
	if event.is_action_pressed("report_bug", false, true) and not ModalLock.any_open(get_tree()):
