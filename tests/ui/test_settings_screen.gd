...
# CheckButton IS-A Button, so this finds the rows as well as Close.
...
func _health_row(screen: Node) -> Button:
	return _button_with_text(screen,
			PlayerSettings.title_of(PlayerSettings.Setting.ALWAYS_SHOW_HEALTH))
...
func test_pressing_the_row_writes_the_preference() -> void:
	SettingsScreen.show_screen(game)
	await _frames(4)
	var screen: Node = _first_modal_of(SettingsScreen)
	var row: Button = _health_row(screen)
	assert_object(row).override_failure_message(
		"the settings page has no row for ALWAYS_SHOW_HEALTH").is_not_null()
	...
	assert_bool(PlayerSettings.is_on(PlayerSettings.Setting.ALWAYS_SHOW_HEALTH)) \
...
func test_the_page_opens_showing_what_was_already_chosen() -> void:
	...
	assert_bool(_health_row(_first_modal_of(SettingsScreen)).button_pressed).is_true()
...
func test_every_declared_setting_gets_a_row() -> void:
	...
	# the property that lets #217's toggle be a store edit rather than UI work.
	SettingsScreen.show_screen(game)
	await _frames(4)
	var screen: Node = _first_modal_of(SettingsScreen)

	for setting: PlayerSettings.Setting in PlayerSettings.Setting.values():
		assert_object(_button_with_text(screen, PlayerSettings.title_of(setting))) \
