...
func test_a_landed_exit_tile_lands_ONCE_however_long_the_flash_holds_the_flight_open() -> void:
	...
	# exit's home is not), and before #602 round 5 every frame past its land time erased and
	# re-added it -- a landing per frame, a version bump per frame, the whole board's props rebuilt
	# per frame. Invisible while the executor tore the flight down at total; the exit now stays
	# open for the flash that covers the camera's drop, which is exactly the window that exposed
	# it.
