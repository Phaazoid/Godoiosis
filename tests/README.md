...
**Exit codes (from gdUnit4's `report_exit_code`):** `0` = clean pass · `100` = test failures **or** errors gdUnit4 itself recorded (a runtime error that aborts a case — e.g. a typed read of a freed instance) · `101` = passed but **orphan nodes** were detected. Treat anything non-zero as "fix it" — see orphan-node hygiene below. **And note `0` can lie too**: gdUnit4 reports it when zero tests ran, which is why the runner has a separate zero-cases guard (next block).

> **A bare `push_error` does NOT red a case — and this table said it did until 2026-09-01.** Measured first in PR #533 (2026-08-25), then again by the ticket that existed *because* of the wrong line: [#668](https://github.com/Phaazoid/Godoiosis/issues/668). `tests/presentation` emitted **four** `ERROR: String formatting error` lines out of `test_card_sheet.gd` and still reported `610 test cases | 0 errors | 0 failures | PASSED`, verdict `0` — on every run, for a fortnight, in every CI log. So **a fault a production path reports by `push_error` is invisible to the suite**: it needs its own assertion, and a green run is not evidence that one exists. That misreading is also how an illegal `SteamRune.tres` shipped for months. (Contrast the freed-instance case above, which gdUnit4 *does* record — as an `error`, not a `failure`, so `0 failures` alone is still not green. The two are different mechanisms and only one of them has teeth.)
...
9. **The content razor (dev rule, 2026-08-15) — rule 8's sibling, for the other thing the dev edits without touching code. A test may LOAD authored content to exercise a real path; it may not ASSERT anything about what that content contains.** *"Authored content shouldn't be pinned, and level geometry, not to mention contents, units, objectives, all of it is authored."* Rule 4 says build content ad hoc and never load a `.tres` — that is the rules layer, where the content IS the subject. `tests/presentation/` is the **declared exception**: those suites load a real mission on purpose, because the bug class they hunt (a wire that never fires — #103, #222) only appears on the real load path. Loading it is the point; asserting on its contents is the error.
   ...
   - **AND AN ABSENCE OF AUTHORED CONTENT IS NOT A FAILURE (dev, 2026-08-27, [#596](https://github.com/Phaazoid/Godoiosis/issues/596)).** His words: *"a lack of authored content shouldn't cause tests to fail - these tests need to be rewritten."* The precondition bullet above says a content precondition is a non-vacuity MESSAGE rather than a magic threshold; this says it may not be an ASSERTION at all when what it demands is that the dev ship a particular *kind* of content. A rename deleted `Super Scope.tres`, the only mod authoring a `family` or a `scaling_change`, and three cases across `test_mod_content.gd` and `test_mod_fitting.gd` reddened — none of them about that mod, every one of them demanding such a mod exist. Three shapes, by what the case is actually for: **a MECHANISM law gets a fixture of its own** (`user://` + `ResourceSaver.save`, read back with `ResourceLoader.CACHE_MODE_IGNORE` — a plain `load()` hands back the very object just saved and the round trip asserts nothing), so it holds on a repo shipping no such content at all; **a SWEEP over shipped content passes vacuously**, because nothing authored violates nothing; **a case that cannot be DRIVEN without content warns and returns** (`push_warning`), keeping the absence audible without calling it a fault. The cost is real and worth saying out loud rather than hiding: a sweep's teeth now follow what is authored, so the picker's family-filter case bites only while some shipped mod is family-locked. Teeth are bought back with a fixture the case OWNS, never with a demand on his content.
   - **AND SO IS A WORD A SURFACE DRAWS (dev, 2026-09-13).** Every example above is a `.tres`, which is exactly why this one got through: the wording on a screen is authored too, he renames it without opening a test, and `Classes/` is not where anyone looks for authored content. `test_the_deployed_strip_spells_every_name_and_squad_title_in_full` recognised a squad-block title by matching the word one said — `label.text == "unsquadded"` — and counted the matches so the sweep could not pass vacuously. Renaming that title to `"solo"` took the count to zero and fired the guard, so **a reworded title was reported as a MISSING one** and every board reddened on a wording change. His words: *"I wanted to change the word that was displayed, but it looks like you had it pinned as a magic number of sorts."* **Exposing the word as a `const` for the test to read is not the fix** — it leaves the test caring about the wording and merely tidies the coupling. **Find a control by its NODE, never by what it says:** the label carries `PreMissionScreen.SQUAD_TITLE_NODE` and the case asks each block for that child, so no rename can reach it, and a block that draws no title now fails BY NAME instead of through a count that could not tell missing from renamed. **This binds a FINDER as hard as an assertion** — a case that locates a button by its caption reds on a rename the same way, and `tests/dev` and `tests/ui` hold about fifteen of those today ("Check board", "Fit", "FIRE", "Wear", `ends_with("(modified)")`), each one a wording change away from a red that names the wrong thing.
...
- **A test that ends in the same frame it queued a free reports FALSE orphans** (measured 2026-08-09, [#162](https://github.com/Phaazoid/Godoiosis/issues/162)). `ScenarioManager.clear_board()` does `remove_child` *then* `queue_free` — deliberately, so same-frame respawns don't see dying units in occupancy checks — and `game.spawn_unit`'s refusal paths do the same. Between those two calls and the next frame, the node is **parentless and pending**, which is exactly what the orphan monitor counts. **The fix is one `await await_idle_frame()` at the top of `after_test()`**, which the board-loading suites carry. It does not blind the check: a genuine leak survives any number of frames and still reports — falsified by planting an unparented `Node.new()` and confirming it was still caught. **The vendored gdUnit4 v6.2.1 ([#482](https://github.com/Phaazoid/Godoiosis/issues/482), 2026-08-24) carries the upstream fix for the parentless-pending class (GD-1291/PR-1292 — queued-for-deletion nodes are filtered out), but the await is STILL load-bearing, measured the same day:** 6.2.1 samples orphans in the same frame as `after_test` (the memory-observer gc() await was deleted), and the filter drops only the queued node itself, **not its descendants** — the orphan list includes children of out-of-tree parents. The dev tools' row repopulates (`remove_child(row)` + `row.queue_free()`) leave a whole queued *subtree* at sample time, so removing the await made 8 dev suites report ~11,200 false orphans on a 0-failure run. The teardown retires when the upstream subtree fix lands ([gdUnit4#1320](https://github.com/godot-gdunit-labs/gdUnit4/issues/1320)); until then a suite that clears the board or repopulates tool rows keeps the await.
...
- **The engine can segfault on SHUTDOWN, after a clean pass** (exit `-1073741819` = `0xC0000005`). Root-caused 2026-07-29 ([#93](https://github.com/Phaazoid/Godoiosis/issues/93)) — **it is a gdUnit4 teardown bug, and it has nothing to do with the code under test.** Findings, each measured:
  ...
  - It is **not fixed upstream**: reproduces unchanged on gdUnit4 **v6.2.0 stable**, and the repo now vendors **v6.2.1** (bumped 2026-08-24, [#482](https://github.com/Phaazoid/Godoiosis/issues/482)) — that bump was never expected to fix it, and the verdict-parsing below is still what makes the exit code truthful.
