...
- **I2 wording** — `SquadManager.disband_squad()` calls `squad.members.erase()` directly, a second eraser besides `_detach_from_current_squad`. (Already noted in squad-system.md / Issues.) The I2 test pins the *observable* detach contract and flags this rather than asserting the internal call path.
...
- **`WeaponData.can_counter` ignored by the counter path — [#34](https://github.com/Phaazoid/Godoiosis/issues/34).** `SquadManager.can_counter` gates on the *component* flag `countering_unit.combat.can_counter` and `has_equipped_weapon()`, never the weapon's own `can_counter` policy field — so the authoring-tool toggle does nothing. Confirmed a bug (the field is meant to be functional). Add a C6-style test (weapon with `can_counter=false` → zero counters) once fixed.
