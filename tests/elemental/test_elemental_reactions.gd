...
func test_water_then_shock_electrocutes() -> void:
	...
	(alch.get_equipped_weapon() as WeaponInstance).template.main_attack.elemental_damage_type = Elemental.Element.WATER
...
func test_e7_counter_can_complete_a_combo() -> void:
	...
	(e.get_equipped_weapon() as WeaponInstance).template.main_attack.elemental_damage_type = Elemental.Element.SHOCK
	...
	plan.counters = _sm.calculate_reactions_for_squad(p.squad, plan.attacks, null)
