...
func test_aoe_counter_splashes_friendlies_in_the_blast() -> void:
	...
	(counterer.get_equipped_weapon() as WeaponInstance).template.attack_pattern = TwoCellBlast.new()
	(counterer.get_equipped_weapon() as WeaponInstance).template.hits_allies = true
...
func test_aoe_counter_without_friendly_fire_spares_allies() -> void:
	...
	_sm.join_squad(ally, counterer.squad)
	(counterer.get_equipped_weapon() as WeaponInstance).template.attack_pattern = TwoCellBlast.new()
	(counterer.get_equipped_weapon() as WeaponInstance).template.hits_allies = false
